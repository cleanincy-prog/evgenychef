...
export default function MenuWorktable() {
  ...
  const source = (width: number) => `/media/menu/worktable/fish-notebook-scene-${width}.webp${suffix}`;
  ...
  return <figure ref={figureRef} className="menu-worktable" data-menu-visual data-ready={state === "ready"} data-state={state}>
    <div className="notebook-scene" aria-busy={state === "loading"}>
      <div className="menu-worktable-picture">
        <img src={source(960)}
          srcSet={`${source(640)} 640w, ${source(960)} 960w, ${source(1419)} 1419w`}
          sizes="min(1120px, calc(90.5vw - 48px))"
          width={1419} height={1108} loading="lazy" decoding="async"
          alt="Раскрытая тетрадь на кремовом столе у светлой стены. Рядом рыба с овощами в глубокой кремовой тарелке с узкой кромкой."
    ...
    <p className="notebook-photo-attribution">Фото томатов: Luc Viatour / Lucnix.be. Фото мидий: Valenzuela400.</p>
