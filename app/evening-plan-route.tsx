  ...
  useEffect(() => {
    ...
    function measure() {
      ...
      if (window.matchMedia("(min-width: 901px)").matches) {
        ...
        if (conversation && menuHeading && board.querySelector(".menu-book")) {
          board.style.setProperty("--menu-book-clearance", `${Math.ceil(Math.max(24, conversation.bottom - menuHeading.bottom + 40))}px`);
        }
        const preparation = board.querySelector(".station-preparation .station-heading")?.getBoundingClientRect();
        const menuBottom = board.querySelector(".menu-book, .menu-plate")?.getBoundingClientRect().bottom;
      ...
      const menuBook = board.querySelector(".menu-book")?.getBoundingClientRect();
      const sauce = board.querySelector(".plate-label-sauce")?.getBoundingClientRect();
      const aisle = sauce ? sauce.left - rect.left - 24 : menuBook ? Math.max(mid, departure + 18) : mid;
      const leaveY = sauce ? Math.max(c.y + 35, sauce.bottom - rect.top + 22) : d.y - 30;
      const departureX = menuBook ? departure : Math.min(departure, aisle - 20);
      const bookTop = menuBook ? menuBook.top - rect.top : 0;
      const outerRail = menuBook ? menuBook.left - rect.left - 22 : 0;
      // The accepted spread spans both columns. Return around its left edge,
      // with the horizontal passage below the conversation drawing.
      const menuReturn = menuBook
        ? `M${b.x - 4},${b.y + 17} C${b.x - 22},${b.y + 42} ${conversationRail},${b.y + 32} ${conversationRail},${b.y + 72} L${conversationRail},${bookTop - 34} Q${conversationRail},${bookTop - 20} ${conversationRail - 14},${bookTop - 20} L${outerRail + 14},${bookTop - 20} Q${outerRail},${bookTop - 20} ${outerRail},${bookTop - 6} L${outerRail},${c.y - 50} Q${outerRail},${c.y - 29} ${c.x},${c.y - 18}`
      ...
      setPaths([
        ...
        arrowTo(menuReturn, { x: c.x, y: c.y - 18 }, menuBook ? { x: outerRail, y: c.y - 29 } : { x: c.x + 16, y: returnY - 8 }),
    ...
    for (const element of board.querySelectorAll(".station-heading, .menu-plate, .menu-book, .plate-label, .conversation-illustration, .preparation-film")) observer.observe(element);
