...
|---|---|---|---|---|
| Коллаж Hero | Прямое «Сделай рызмытие 6» и выпуск 114 | Точное значение пользователя | Сохранить композицию и контент | Статический blur(6px), при движении none |

## Более сильное размытие Hero — 2026-09-22

| Элемент | Источник | Принцип | Ограничение | Реализация |
|---|---|---|---|---|
| Коллаж Hero | Действующий выпуск 113 и «Еще больше размытости» | Усилить отделение фоновых фото | Сохранить все фото, тексты и композицию | Статический blur 5 → 8 px; во время движения none |

## Ещё мягче коллаж Hero — 2026-09-22

Прямое уточнение пользователя усиливает прежний фильтр 3 → 5 px. Референс — утверждённый Hero; нового оформления и композиции нет. [Аудит](HERO_COLLAGE_BLUR_5PX_2026-09-22.md).

## Размытие коллажа — 2026-09-22

Референс — существующий Hero и прямой запрос немного усилить blur. Меняется только статический фильтр 2 → 3 px, без нового декора или композиции. [Аудит и план](HERO_COLLAGE_BLUR_2026-09-22.md).

## Запуск Hero на iPhone — 2026-09-22

Источник — замечание пользователя и утверждённый сценарий Hero справа налево за 3 секунды. Меняется совместимость и ожидание ресурсов; фото, итоговая геометрия и оформление сохраняются. Новых визуальных решений без источника нет. [Аудит и план](HERO_IPHONE_2026-09-22.md).

## Полная сцена знакомства — 2026-09-22

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Низ сцены «Знакомимся» | Выбранный пользователем рисунок ImageGen и прямое требование сохранить анимацию | Полный стол, стулья, ноги как продолжение исходного движущегося верха | Статичные лица и постоянная реплика из рисунка | Прозрачная нижняя дорисовка поверх прежнего MP4, ConversationVideo |

[Аудит и план](CONVERSATION_SCENE_2026-09-22.md).

## Публикация согласованного Hero — 2026-09-22

Референс движения — точное пользовательское описание справа налево за 3 секунды; финальная композиция — действующий Hero из 63 фото. Новые декоративные элементы и библиотечное оформление не добавляются. Источники изображений и текст сохраняются. [Аудит, границы и проверки](HERO_RELEASE_2026-09-22.md).

## Короткий заголовок об опыте — 2026-09-22

Прямой текст пользователя и существующий award-proof: «Больше 20 лет работы шеф-поваром». Только редакция h2, без новых визуальных решений. [Аудит и проверки](EXPERIENCE_HEADING_2026-09-22.md).

## Более заметная лесенка этапов — 2026-09-22

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Этапы «От разговора — к вашему столу» | Прямое уточнение о блоках ниже друг друга; действующий маршрут и согласованная лесенка 13 сентября | Правый этап каждой пары начинается ниже левого; на телефоне последовательный поток | Новые карточки, декоративные эффекты, изменение контента | Отступы в `globals.css`, измерение в `evening-plan-route.tsx` |

60% — рабочая численная интерпретация пожелания о более заметном смещении. [Аудит и проверки](PROCESS_STAGGER_2026-09-22.md).

## Воспроизведение знакомства — 2026-09-22

| Элемент | Референс | Принцип | Что не копируется | Реализация |
|---|---|---|---|---|
| Видео этапа 01 | MP4 пользователя и прямое уточнение о прокрутке/двух повторах | Прежний полный кадр, автоматическое воспроизведение, максимум два просмотра | Бесконечный loop и ожидание полного появления | ConversationVideo, отдельный hook, облегчённый MP4 |

[Аудит](CONVERSATION_PLAYBACK_2026-09-22.md). Новых визуальных решений нет.

## Без нумерации продуктов и этапов — 2026-09-22

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Продукты и этапы | Прямое указание пользователя убрать 01–03 и 01–04; действующие ChefSourcing и StationHeading | Удалить семь номеров и занимаемые ими колонки, сохранить тексты и прежнее оформление | Новые декорации и переработка других блоков | `app/chef-sourcing.tsx`, `app/page.tsx`, `app/globals.css` |

## Абзац об авторской кухне — 2026-09-22

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Текст об опыте шефа | Дословный текст пользователя; действующий award-proof | Персональная биография и связь с тремя форматами | Предыдущие варианты редакции, новые обещания и оформление | Один существующий p.award-note |

[Точный текст и аудит](BIOGRAPHY_COPY_2026-09-22.md). Визуальных решений без источника нет.

## Видео работы над меню — 2026-09-22

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Шеф с тетрадью | Пользовательский MP4 и снимок 22 сентября 18:10 | Полная сцена 16:9, плавное движение руки, прежняя ширина | Аудиодорожка, обрезка, новая генерация | MenuWorktable с 60 fps / 0.8× и постером из первого кадра |
| Воспроизведение | Одобренные видео сайта / usePageMediaPlayback | По видимости, повтор, reduced motion, постер и повтор при ошибке | Панель управления, новая библиотека | Тот же hook, прежняя кнопка ошибки |

[Аудит и план](MENU_VIDEO_2026-09-22.md). Визуальных решений без источника нет.

## Видео «Знакомимся» — 2026-09-22

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Разговор шефа и гостьи | Пользовательский Оживи_приложенную_карандашную.mp4 | Точная выбранная анимация и встроенная реплика | Аудиодорожка, панель управления, обрезка кадра | ConversationVideo внутри прежней conversation-illustration |
| Воспроизведение и ошибка | Одобренные ChefStoryVideo / usePageMediaPlayback | Запуск по прокрутке, повтор, пауза вне экрана, reduced motion, кнопка только при ошибке | Новая библиотека или плеер | Тот же hook и оформление кнопки, неподвижный постер |

[Аудит](CONVERSATION_VIDEO_2026-09-22.md). Неутверждённых визуальных решений нет.

## Краткий Hero — 2026-09-22

Прямое указание пользователя удаляет строку услуг. Референс — существующий одобренный Hero: h1, портрет, коллаж и полноэкранная высота сохранены. Новых визуальных решений нет. [Аудит](HERO_COPY_CLEANUP_2026-09-22.md).

## Заголовок тарелки — уточнение 2026-09-20

Прямая редакция пользователя меняет только «Как складывается вкус» на «Как формируется вкус». Источник остального DishNotes — восстановленный выпуск 95; композиция прежняя.

## Возврат блока с тарелкой — 2026-09-20

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| DishNotes целиком | Выпуск 95, commit f6f7c63; показанный пользователю previous-dish-mobile.png | Точный прежний заголовок, четыре подписи, фото, три выноски и положения | Прежние редакции остальных разделов | Восстановление только app/dish-notes.tsx; CSS и медиа уже совпадают |

[Аудит](RESTORE_DISH_BLOCK_2026-09-20.md). Новых визуальных решений нет.

## Финальная редакция пользователя — 2026-09-20

| Элемент | Источник | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Все основные тексты | Последнее сообщение пользователя | Его точные слова и порядок | Предыдущие предложения агента | page, ChefSourcing, DishNotes |
| Услуги в Hero | Та же первая фраза; действующая светлая область Hero | Читаемая типографическая иерархия | Новый баннер, кнопки или композиция | Обычный абзац под прежним пятистрочным заголовком |

[Аудит до кода](FINAL_COPY_2026-09-20.md). Остальная композиция и медиа сохраняются.

## Тетрадь: утка на 12 человек — 2026-09-20

| Элемент | Референс | Принцип | Что не копируется | Реализация |
|---|---|---|---|---|
| Записи в тетради и на листах | Новый пользовательский снимок; approved-chef-menu-2026-09-14.png; состав уже выбранного Atlas-фото | Тот же шеф и рисунок, содержательная связь с тарелкой ниже | Прежние треска, мидии и расчёт на 4; неизвестная рецептура ресторана | Только новое рукописное содержание и эскиз подачи; иллюстративные пропорции на 12 человек |

[Аудит и расчёт](DUCK_NOTEBOOK_2026-09-20.md). Новых композиционных решений нет.

## Удаление двух подписей — 2026-09-19

Прямой запрос удаляет подпись автора под тарелкой и заключение «Знать продукт. Выбирать самому.». Сохраняется одобренная композиция; исчезают только два текстовых узла и отведённые им строки. Новых визуальных решений нет. [Аудит](CAPTION_CLEANUP_2026-09-19.md).

## «Как складывается вкус» — 2026-09-19

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Тарелка и пояснения | Одобренный atlas-dish-notes.png, затем прямое «Добавь на сайт» | Кремовая тарелка, четыре подписи от третьего лица, три тонкие выноски | Предыдущие восемь подписей и речь от первого лица | Изображение отдельно от HTML-текста |
| Место на странице | Смысл этапа 02 «Продумываю меню» и существующая схема процесса | Пояснение сочетания вкусов сразу после составления меню | Прежние удалённые меню, цены, FAQ | Полная ширина под рисунком шефа внутри этапа 02 |
| Телефон | Согласованная компактность и читаемость сайта | Две заметки над тарелкой, две под ней | Уменьшенная копия desktop-картинки с мелким текстом | Адаптивная сетка, при очень крупном тексте естественный перенос |

[Аудит и проверка](DISH_NOTES_2026-09-19.md). Отдельного мобильного референса нет; адаптация следует действующей типографике и содержимому.

## Компактная группа «Почерк шефа» — 2026-09-19

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Портрет и вводный текст | Выбранный макет 6 и новая критика двух мобильных скриншотов | Рисунок рядом с текстом, компактная цельная группа | Длинная вертикальная стопка выпуска 91 | На телефоне две колонки только для портрета и введения, продукты ниже на всю ширину |
| Масштаб и отступы | Прямое «Очень растянуто» и действующая типографика | Умеренный рисунок и плотные открытые строки | Уменьшение всей страницы и мелкий текст | Основной мобильный текст от 17 px, естественная высота |

## Публикация выбранного sourcing-портрета — 2026-09-19

| Элемент | Источник | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Общая композиция | Новый скриншот пользователя и 06-portrait.html | Центрированный заголовок, портрет слева, три открытые строки справа | Шапка/подвал примерки, дополнительные виньетки, чертежи | Самостоятельный раздел после форматов |
| Рисунок | Выбранный chef-landscape.png | Портрет и места внутри силуэта целиком | Новые генерации и обрезка | Точный оригинал и адаптивные WebP |
| Текст | Прямой запрос акцента на мясо, рыбу и морепродукты | Продукты в заголовках, места в описаниях | Неизвестные поставщики и имя фермы | Новая редакция в аудите |
| Адаптивность | Действующие 16–24 px / 82.5rem и выбранная примерка | Согласованный масштаб и естественная высота | Узкая неизменная колонка на большом экране | Контейнерная компоновка, одна колонка на телефоне |

[Аудит](CHEF_SOURCING_2026-09-19.md). Размещение между форматами и процессом — функциональная адаптация нового блока к действующей странице; отдельного референса расположения пользователь не предоставлял.

## Исправление пропорций после замечания 16:08 — 2026-09-18

| Элемент | Источник | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Ширина и читаемость на большом экране | Новый снимок пользователя 3125×1707 и критика выпуска 89 | Текст и геометрия растут согласованно | Неподвижная колонка 1320 px на любом мониторе | Масштаб 16–24 px и общий контейнер 82.5rem |
| Фото и описание | Выбранный «Альбом шефа», новое замечание о результате | Единая визуальная пара и читаемая подпись | Небольшой лист у верхнего края выросшего фото | Крупнее текст, вертикальное центрирование, прежний нахлёст |
| Опыт и заголовки | Согласованный текст и действующие шрифтовые роли | Фраза об опыте получает явную иерархию | Новые слова или отдельный маркетинговый блок | Oranienbaum h2 и прежние абзацы |

[Аудит и план](ALBUM_PROPORTIONS_2026-09-18.md). Исправляется размер, а не выбранный стиль.

## Полный первый экран и общие границы разделов — 2026-09-18

| Элемент | Источник | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Высота Hero | Прямое напоминание и снимок пользователя 14:53:47; прежнее требование полного экрана | Обложка и шапка заполняют первый viewport на любой ширине | Ограничение 824 px и изменение высоты от скролла | Минимум через 100svh с учётом шапки |
| Разделы после Hero | Замечание о расположении; действующая общая сетка до 1320 px | Самостоятельный опыт шефа, общие края и интервалы разделов | Вложенное сужение альбома до 1000 px на desktop | Существующие опыт, форматы, процесс и контакт |
| Альбом | Выбранный `chef-album-approved-2026-09-18.html` | Фото/лист, малый нахлёст, чередование; отдельная мобильная компоновка | Новые сцены и оформление из отклонённых вариантов | Прежние элементы альбома в общей ширине |

[Аудит и план до кода](HERO_PAGE_FLOW_2026-09-18.md). Новых визуальных решений без источника нет.

## Выбранный альбом форматов — 2026-09-18

| Элемент | Подтверждённый источник | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Три формата вечера | Прямой выбор пользователя; `design/references/chef-album-approved-2026-09-18.html` | Крупные фото, светлые листы, небольшой нахлёст, чередование второй строки | Три невыбранных направления, карандашные предметы четвёртого варианта, новые тексты и функции | Существующие `.fmt` и `.format-caption` |
| Текст и фото на телефоне | Мобильная компоновка выбранного макета | Фото 95%, лист 94% со сдвигом и нахлёстом только у нижнего края, свободная высота | Уменьшенный настольный разворот и горизонтальный скролл | Адаптивные правила до 760 px |

[Аудит, план и результаты](CHEF_ALBUM_2026-09-18.md). Остальные активные источники остаются прежними.

## Полный предпросмотр ссылки — 2026-09-18

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Обложка ссылки | Прямой запрос; текущий Hero, реальный chef-hero-apron.jpg и действующие токены | Портрет с именем и ясной услугой; светлая бумага, обычный serif, читаемость миниатюры | Старые зелёные/рыжие диагонали, массивный гротеск, телефонный интерфейс исходника | Один новый OG bitmap с новым URL |
| Title, description, OG/X | Действующие факты app/page.tsx | Сразу объяснить, кто шеф, где и какие услуги | Внутреннее название концепции, цены, число гостей, придуманные достижения | Общие константы, одинаковая информация в метаданных |
| Значок сайта | Существующая монограмма ЕГ; действующие Oranienbaum и paper/ink | Узнаваемые инициалы в текущем оформлении | Старая зелёно-рыжая палитра, новая эмблема | Нативный SVG на основе настоящих глифов шрифта |

[Аудит и проверка](LINK_PREVIEW_2026-09-18.md).

## Видео без панели управления — 2026-09-15

Источник — прямое «убрать стоп, перемотку и т.д., оно должно просто воспроизводится» и действующий вертикальный film-stage. Удаляется штатная панель; сохраняются кадр, постер, пропорции и позиция запуска. Новых визуальных паттернов нет. Прежнее требование ручного управления отменено этим запросом. [Аудит и проверка](QA_FIXES_VIDEO_2026-09-15.md).

## Текст об опыте шефа — 2026-09-15

| Элемент | Источник | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| award-proof | Прямая редакция пользователя и существующий опубликованный блок | Заголовок про 20 лет работы шеф-поваром и четыре абзаца в прежнем оформлении | Прежние 25 лет и старые формулировки | app/page.tsx, существующие award-title / award-note |

[Точный текст и аудит](ABOUT_CHEF_COPY_2026-09-15.md). Новых визуальных решений без источника нет.

## Редакция мастер-классов и «Ваш вечер» — 2026-09-15

Источник — прямое сообщение пользователя с двумя заменами текста. Переносятся участие, техники и дегустация в описании мастер-классов; в этапе 04 остаются время с гостями и приготовление/подача шефом. Исправлена очевидная опечатка в слове «дегустацию». [Аудит и точные тексты](MASTERCLASS_EVENING_COPY_2026-09-15.md).

## Новое описание частного ужина — 2026-09-15

Единственный источник — дословная редакция пользователя с окончанием «авторским ужином в 7 подач». Существующая карточка формата сохраняется; переносится только текст. [Аудит](PRIVATE_DINNER_COPY_2026-09-15.md).

## Удаление контактной подсказки — 2026-09-15

Прямое «Удали \"в первом сообщении укажите дату и формат\"» удаляет абзац contact-help. Кнопка Instagram и аккаунт остаются в прежнем оформлении, без пустого контейнера под подсказку. [Аудит](CONTENT_CLEANUP_2026-09-15.md).

## Загрузка Hero без подмены шрифтов — 2026-09-15

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Первый экран | Прямое сообщение о смене шрифта и скачках фото; существующий опубликованный Hero | С первого показа те же шрифты и стабильная геометрия | Запасное начертание, дополнительная строка и сдвиг портрета | Inline WOFF2 в CSS, проверка кадров загрузки |
| Портрет и коллаж | Действующие оригиналы и адаптивная сетка | Прежняя композиция и резервирование места до загрузки | Новые фото, фиксирование высоты текста вопреки 200% увеличению | [Аудит и проверка](HERO_LOADING_2026-09-15.md) |

## Удаление условий и лишнего текста — 2026-09-15

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Условия форматов | Прямое «Удали время, места» | Только фото, название и прежнее описание | Время и место из предыдущей итерации | Удаление полей и fmt-spec |
| Подпись видео | Прямое «описание у видео под видео» | Самостоятельный плеер | Обычная подпись | Текст остаётся только при ошибке загрузки |
| Пример меню | Прямое «блок, меню вашего вечера и блюда» | Удаление всего отдельного блока | Заголовок, вводный текст, блюда и стили блока | Удаление sample-menu и sampleDishes |
| Контакт | Прямое удаление «Начнём с вашего вечера» и прежняя мобильная компоновка | Одна колонка без пустого заголовка, до 510 px | Пустая левая колонка и разделитель | Прежняя кнопка Instagram, аккаунт и подсказка |

Новых решений без источника нет. [Аудит и проверка](CONTENT_CLEANUP_2026-09-15.md).

## Отмена FAQ и числовых условий — 2026-09-15

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| FAQ и тарифы | Прямое «удали этот блок… убери вообще цены и количество гостей» | Удаление отклонённого содержания | FAQ, цены, гостевые числа и их заглушки | page.tsx, globals.css |
| Фото мастер-класса | Пользовательский снимок ошибки; прежний оригинал masterclass.jpg | Тот же кадр на прежнем месте, корректный JPEG MIME | Новая фотография или генерация | JPEG responsive variants, manifest и генератор |

[Аудит и проверка](FORMAT_CLEANUP_2026-09-15.md). Эта запись отменяет добавление FAQ и цен из предыдущей итерации.

## Меню, форматы и FAQ — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Пример меню | Прямой menu-formats-faq-brief | Номер моно, имя display, сопровождение sans, настоящий курсив | Список на бумагах не превращается в утверждённое меню | Семантический список подач, отдельная строка на телефоне |
| Форматы | Тот же brief и существующие одобренные фото/описания | Сопоставимые строки, четыре спецификации, цена последняя золотом | Выдуманные цены и вместимость; наклон текста | Общая вертикаль цен, мобильная сетка 2×2, прежние фото-стикеры |
| FAQ | Прямой brief и действующий текст сайта | Открытые вопросы Ysabeau 600, ответы 400 | Аккордеон, неподтверждённые условия услуг | Обычные заголовки и абзацы, ответы до 52ch |

[Аудит, план и проверки](MENU_FORMATS_FAQ_2026-09-14.md). Рисунок шефа и этап 02 сохраняются. Все новые визуальные решения имеют прямой источник.

## Выбранный шеф с тетрадью на сайте — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Сцена 02 | Прямое «Добавляй на сайт», исправленный chef-menu-copy | Один графитный шеф над рабочими листами, точные новые надписи | Цветная тарелка и прежний фотомонтаж | MenuWorktable, WebP выбранного PNG |
| Положение | Выбранная HTML-примерка, прежняя лесенка процесса | Правая колонка, до 620 px, отступ 34/17 px | Широкий разворот на обе колонки | Обычная сетка процесса, соответствующий обход стрелки |
| Внешний текст | Прямое уточнение о надписях в руках | Исходные заголовок и абзац | Неутверждённая новая редакция сайта | page.tsx без изменения копирайта |

[Аудит и выпуск](MENU_CHEF_NOTES_SITE_2026-09-14.md). Существующий публичный домен и аудитория сохраняются.

## Уточнение: надписи в тетради, исходный текст этапа — 2026-09-14

Прямое уточнение пользователя ограничивает редактуру надписями внутри рисунка. Опора для восстановления заголовка и абзаца — исходная примерка `chef-menu-no-plate-2026-09-14`. Новая редакция этих внешних текстов не выбрана. Обновлённая тетрадь показывается крупно, без нового растрового редактирования. [Аудит исправления](../design/mockups/chef-menu-copy-2026-09-14/README.md).

## Полная редакция текста макета — 2026-09-14

По «Текст надо поменять 100%» подготовить собственную новую редакцию записей и текста этапа. Основа композиции — одобренная сцена без тарелки. Рабочие формы и числовые данные сохраняют прежние реальные источники; новые названия и фразы опираются на прямой запрос и содержание этапа. [Текст и аудит](../design/mockups/chef-menu-copy-2026-09-14/README.md). Личные данные гостей и фактическая закупка не придумываются.

## Одобренная сцена, убрать тарелку — 2026-09-14

Прямое «Выглядит отлично» одобряет внешний вид последнего макета; «текст другой» оставляет содержание на доработку. По «И тарелку убери с едой» исключается отдельный фотослой. Графитная сцена и прежний CSS сохраняются без перерисовки. [Аудит](../design/mockups/chef-menu-no-plate-2026-09-14/README.md).

## Реальные рабочие документы — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Тетрадь | Реальные фотографии StarChefs, Jeong / Mallahan | Плотные короткие записи, линейка, обычный рабочий почерк | Чужие страницы, почерк и идентичность | Новая иллюстрация выбранного ракурса |
| Заготовки и закупки | Chef’s Resources, Prep Sheets | Потребность → сверка остатка → подготовка/заказ | Выдуманные остатки, цены и поставщики | Незавершённый лист сверки |
| Проверяемые количества | Steve Groves / Great British Chefs, 4 порции | Данные опубликованного примера, явная атрибуция | Приписывание рецепта Евгению или фото пользователя | Фрагмент рецептуры и расчёт 600 / 4 |

[Источники и аудит](../design/mockups/chef-menu-real-notes-2026-09-14/README.md). Ракурс третьего варианта сохраняется; декоративные величины прошлого расширения отклонены. Только макет.

## Выбран вариант 3, расширены рабочие материалы — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Ракурс и человек | Прямое «Мне нравится третий вариант» и option-3-mockup.png | Вид через плечо, руки у обычной тетради, графит | Другие невыбранные ракурсы | Основа новой отдельной примерки |
| Объём работы | «Больше… страниц, чертежей… работа шефа сложная и объёмная» | Разные задачи листов, наброски и исправления | Случайные формулы, бессмысленное нагромождение | Подача, порция, порядок и сочетания |
| Настоящая рыба | Сохранённое пользовательское фото | Цветной результат рядом с черновой работой | Генерация или ретушь еды | Отдельное изображение на переднем краю стола |

[Аудит и эскиз](../design/mockups/chef-menu-studies-2026-09-14/README.md). Выбрана основа третьего варианта; расширение пока показано как макет, сайт не меняется.

## Вариант 1: меню в руках — 2026-09-14

«А нарисуй первый вариант)» поручает дополнительный [эскиз](../design/mockups/chef-menu-options-2026-09-14/README-option-1.md): шеф читает обычный лист меню у груди, держит карандаш у строки. Внешность и манера — из реального портрета и одобренного разговора; цветная рыба — прежний пользовательский фотослой. Масштаб, поза и отдельный лист соответствуют прямо заказанному варианту. Гигантская тетрадь, закупки и новая стилистика не переносятся. Выбор и интеграция пока не выполнены.

## Два новых ракурса работы над меню — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Выбор блюд из листков | Прямое «Нарисуй второй и третий вариант», предложенный вариант 2 | Шеф отбирает несколько подач, жест выбора виден | Огромная тетрадь, закупки | Отдельная графитная сцена |
| Вид через плечо | Тот же запрос, предложенный вариант 3 | Наблюдаем руки и записи со стороны шефа | Фронтальный портрет, крупная книга перед зрителем | Отдельная сцена с реальным фото рыбы справа |
| Внешность и рисунок | Реальный chef-hero-apron.jpg, одобренный conversation-960.webp | Евгений, тонкий реалистичный графит на paper | Гостья и прежняя фронтальная композиция | Два запроса встроенного ImageGen |

Оба результата — [эскизы для сравнения](../design/mockups/chef-menu-options-2026-09-14/README.md), не выбор пользователя и не обновление сайта.

## Шеф составляет меню — уточнение макета, 2026-09-14

Этот вариант отклонён прямым «Нет, так не подходит, думай другие варианты». Ниже сохраняется история источников, а не выбранное направление. Новый поиск композиции пока на стадии идей.

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Понятное действие шефа | «Ну как тогда понятно, что он меню занимается?»; выбранный рабочий стол с эскизами | Крупное меню, названия блюд, карандаш над рисунком подачи | Пакет, список закупок, галочки | Отдельная переработанная иллюстрация |
| Цветная подача | Пользовательская фотография рыбы | Настоящая тарелка рядом с набросками | Генерация или ретушь еды | Отдельный исходный фотослой в HTML-примерке |
| Контекст | Существующие шрифты, бумага и композиция процесса | Узнаваемое оформление сайта | Подключение к активному приложению | [Макет и аудит](../design/mockups/chef-menu-planning-2026-09-14/README.md) |

## Шеф продумывает закупки — отдельный макет, 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Шеф с тетрадью | Прямой запрос, `chef-hero-apron.jpg` | Узнаваемый Евгений, естественная задумчивая поза, рабочий список | Постановочная улыбка, новый персонаж | Новая иллюстрация для отдельной примерки |
| Манера рисунка | Одобренный `conversation-960.webp` | Тонкий графит, анатомия, незавершённые внешние линии | Глянцевый 3D, мультфильм, пузыри мыслей | Встроенный ImageGen по референсу |
| Контекст сайта | Актуальные page.tsx / globals.css | Текущие цвета, шрифты, первые два этапа | Редизайн сайта и публикация эскиза | Отдельный HTML-макет |

[Аудит и границы](../design/mockups/chef-purchasing-2026-09-14/README.md).

## Возврат к выбранной композиции — 2026-09-14

Последнее прямое задание сохраняет весь `menu-worktable-v1.png` и убирает содержимое передней тарелки для дальнейшего фотослоя. Предыдущий фотографический монтаж ниже отклонён. Тетрадь, свет, реквизит, надписи и карандашные рисунки исходного макета — действующий визуальный ориентир для этой подложки. [Результат](../design/mockups/menu-approved-empty-plate-2026-09-14/README.md). [Сравнение и самостоятельно найденный референс Eatery Berlin](../design/references/menu-pen-reference-2026-09-14/README.md): принцип тонкой ручки и рабочих выносок, без копирования чужого рисунка. Активный UI не изменён.

## Настоящие предметы — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Сцена тетради | Выбранный пользователем `menu-worktable-v1.png`; новый запрос реализма | Наклонённая тетрадь, готовая тарелка рядом, рабочие записи | Сгенерированные бумага, еда, посуда | Реальные фотослои с согласованным верхним ракурсом |
| Фактура предметов | [Markus Spiske, Alorin, Pexels](../design/references/menu-photographic-2026-09-14/SOURCES.md) | Настоящие бумага, сгиб, керамика, карандаш, светлая поверхность | Чужой бренд, ноутбук, тёмный стол | Маски и физические контактные тени, paper сайта |
| Продукты на правой странице | Реальные исходники; прямое требование фотографий | Нормально узнаваемые продукты, вид фотографий на бумаге | Тонкие обрывки и неприятный прежний силуэт мидии | Целый пучок моркови, горох, мидия в раковине; другие реальные фото сохранены |
| Надписи и mobile | Рукописная тетрадь выбранного эскиза; «всё аккуратно в одном блоке» | Живые заметки внутри кадра; две страницы и чаша всегда вместе | Раскрытие, перелистывание, три вертикальных кадра | Caveat только на бумаге, обычная внешняя подпись, текущие breakpoints |

[Аудит и план](MENU_PHOTOGRAPHIC_2026-09-14.md). Координаты определяются реальными контурами; нового стиля сайта нет.

## Цельный блок на телефоне — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Тетрадь на телефоне | Три снимка пользователя и прямое «всё аккуратно в одном блоке, не шире скрола»; одобренный цельный рабочий стол | Обе страницы и тарелка в одном кадре, в пределах ширины экрана | Отклонённые последовательные страницы и отдельный кадр тарелки | Единая сцена с пропорциональными координатами текста и фото |
| Чтение и взаимодействие | Прямое «Не надо ничего разворачивать» | Название и подпись читаемы снаружи изображения; без раскрытия и горизонтальной прокрутки | Предложенный просмотрщик, масштабирование по нажатию | Обычная подпись в текущих шрифтах, доступное содержимое тетради |

[Аудит и план](MENU_NOTEBOOK_ONE_BLOCK_2026-09-14.md). Предыдущая запись о вертикальных страницах ниже отклонена пользователем.

## Страницы ингредиентов и глубокая тарелка — 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Левая и правая страницы | Прямой запрос; выбранный рабочий стол | Описание и состав слева, настоящие фотографии компонентов справа | Прежний список четырёх блюд, карандашные рисунки | Живой HTML и реальные фотослои поверх чистого разворота |
| Окружение и посуда | Прямой запрос; палитра сайта | Стена и стол внутри изображения; глубокая посуда цвета paper | Широкая белая плоская тарелка, отдельный фон раздела | Новая подложка, фон раздела #f4efe5 |
| Готовая еда | Присланное фото рыбы | Исходные цвет и фактура, прозрачная маска | Генерация еды, неподтверждённый вид рыбы | Исходный фотослой внутри новой тарелки |
| Страницы на телефоне | Заданные роли страниц; типографика сайта | Каждая страница сохраняет полный размер текста | Мелкие растровые надписи | Последовательные страницы, фотографии парами, подача ниже |

[Аудит и план](MENU_WORKTABLE_INGREDIENTS_2026-09-14.md). Точные маски и геометрия страниц адаптируются к подложке.

## Присланная рыба — выбрана, 2026-09-14

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Цветная тарелка | Фото пользователя от 2026-09-14 | Сохранить настоящие продукты, фактуру, подачу и цвет | Чёрные поля телефона, деревянный фон вокруг тарелки | Контурная маска, пропорциональный размер, отдельная тень; без генерации еды |
| Строка меню и набросок | То же фото; одобренный `menu-worktable-v1.png` | Фото и эскиз изображают одно блюдо | Утка, неподтверждённый вид рыбы | «Рыба с овощами», нижний графитный набросок |
| Размер и место сцены | Действующий MenuWorktable | Полный кадр 3:2 и читаемая подпись | Новый шаблон раздела, новая анимация | Существующие компоненты и адаптив |

[Аудит и план](MENU_WORKTABLE_FISH_2026-09-14.md).

## Новая фотография утки — подбор, 2026-09-13

Прежний монтаж с Deuxave отклонён пользователем. [Аудит и источники](../design/references/duck-worktable-selection-2026-09-13/README.md). Кандидат ещё не утверждён.

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Подача утки | [BOKA / Will Blunt, StarChefs](https://www.starchefs.com/on-the-ground/jonathan-dockter) | Объём грудки, матовая светлая керамика, спокойный гарнир | Чужой стол, утверждение о блюде Евгения | Локальная фотография и отдельная примерка; сайт прежний |
| Рабочий стол | Одобренный `menu-worktable-v1.png` | Ракурс, масштаб тарелки, общий мягкий свет | Автоматическое признание любого монтажа соответствующим макету | Визуальное сопоставление в отдельном эскизе |

## Создание меню вместо осьминога — 2026-09-13

**Интеграция в localhost:** после «На сайте сделал?» выбранный исправленный эскиз переносится в этап 02. [Аудит и план](MENU_WORKTABLE_SITE_2026-09-13.md). Сцена до 960 px сохраняет полный кадр; обычный HTML-текст повторяет пример меню для маленького экрана и доступности. Сетка, шрифты и стрелки происходят из действующего интерфейса. Основной домен этой итерацией не меняется.

**Актуальное блюдо — исходная фотография утки:** «Нет, утку которая была на сайте, её делай, не рисованную». Используется `public/media/web/duck-plate-960.webp`, отдельным слоем без генерации и ретуши еды. Прежний нарисованный атлас отклонён. [Исправленный эскиз и проверка](../design/mockups/menu-worktable-2026-09-13/real-duck/README.md).

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Цветная утка | Исходное фото сайта `duck-plate-960.webp`, прямое уточнение | Сохранить настоящую фактуру, исходную подачу и соусы | Генеративная замена еды, состав ошибочного атласа | Пропорциональное уменьшение и отдельный фотослой; 0 отличий непрозрачных пикселей после уменьшения |
| Рабочий стол | Одобренный `menu-worktable-v1.png` | Прежний блокнот, пример меню, предметы и ракурс | Новая композиция и стили сайта | Подложка с убранной прежней тарелкой; соответствующий рисунок на бумаге |

**Выбран рабочий стол:** «Отлично, да, кайф, сделай так с осьминогом» утверждает композицию [menu-worktable-v1.png](../design/mockups/menu-worktable-2026-09-13/menu-worktable-v1.png) и запрашивает замену лосося на осьминога. Принцип — неизменная предметная сцена; состав новой подачи взят из действующего осьминога (томаты, оливки, томатный соус, зелень). Меняются готовая еда, её нижний набросок и соответствующие надписи. Новая композиция или декоративные элементы не вводятся.

«Нарисуй как это будет выглядеть» разрешает отрисовку предложенного рабочего стола. [Отдельный эскиз](../design/mockups/menu-worktable-2026-09-13/README.md): раскрытый блокнот, пример меню и наброски подач, одна тарелка, лимон, веточка и карандаш. Принцип графит/цвет — из пользовательского рисунка, палитра — из сайта; композиция рабочего стола — прямое продолжение предложенной идеи, которую пользователь поручил визуализировать. Статус остаётся «эскиз», не утверждённый интерфейс.

Первый эскиз отклонён. Последующий запрос лучшей идеи: предложен рабочий стол шефа с раскрытым блокнотом, карандашными пометками, отдельными продуктами и одной готовой тарелкой. Это концептуальное предложение по мотиву блокнота исходного референса, не утверждённая композиция. Ни одна из новых картинок пока не служит одобренным источником для кода.

**Только эскиз:** последующее «Сначала эскиз потом всё остальное» откладывает интеграцию. Ниже описано предложенное направление отдельного макета; действующий интерфейс пока сохраняет осьминога.

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Сцена 02 | [Референс пользователя](../design/references/menu-concept-2026-09-13/user-reference.jpg), прямой запрос | Разговор → эскизы шефа → ингредиенты → меню; карандаш и цветная еда | Чёрные поля скриншота, чужой персонаж, буквальная копия рисунка | Оригинальная иллюстрация с Евгением на бумаге сайта |
| Четыре подачи | Тот же референс | Блюда объединены историей меню | Обязательное меню, цены, чужие фотографии | Иллюстративные гребешки, велюте, лосось и панна-котта; HTML-подписи и пометка «пример» |
| Лицо и стиль | chef-hero-apron.jpg, действующий карандашный рисунок 01 | Узнаваемый Евгений и тонкий графит | Женский персонаж референса | ImageGen с отдельными композиционным и портретным источниками |
| Шрифты и адаптация | Действующая система и сетка процесса | Одна сцена до 960 px; четыре подачи, на телефоне две пары | Книжная оболочка, карточки, новые шрифты, анимация | MenuConcept; прежние 40%/+55 px и измеряемая стрелка |

Геометрия адаптации и краткие названия — редакционные решения внутри прямого запроса. Нового общего стиля без источника нет. [Аудит и план](MENU_CONCEPT_2026-09-13.md). Ниже — история.

## Постоянный цвет и больше ингредиентов — 2026-09-13

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Цветное меню | Прямое «убери идею с отрисовкой, просто оставь цветную версию» и принятый разворот | Все продукты и готовые блюда доступны сразу | Карандаш, скролл-переходы, переключатели видов | Только цветные изображения в HTML |
| Расширенный состав | Прямые «добавь больше ингредиентов» и «Соль, перец, граммовки добавь», существующий рецепт | Десять компонентов с действиями и количеством на 1 порцию | Новые фантазийные изображения и изменение подачи | Десять фото-компонентов, включая соль и перец; изображение каждой позиции по последующему прямому уточнению |
| Единственный лист | Последующее «Оставь только осьминога», утверждённый лист и действующая система | Один рецепт, фото/приготовление, тарелка ниже | Утка, каре, перелистывание и пустой соседний лист | Один лист шириной до 640 px, на mobile по доступной ширине |

Изображения каждого ингредиента — прямое «Картинки к ингредиентам нужны везде». Прежние четыре фото и тарелка осьминога сохраняются. Новые источники: Kaboompics/Pexels5202104 (базилик, существующий актив), Jon Sullivan/Wikimedia Commons (чеснок и перец), Pixabay/Pexels33783 (масло), André Karwath + Andrewc/Wikimedia Commons (лимон), Poyraz72/Wikimedia Commons (соль). Принцип — натуральные предметные фрагменты на листе, без чужой композиции, кухонного окружения и новых декоративных рамок. Полные ссылки и лицензии — в public/media/menu/exploded/credits.html; из листа ведёт нейтральная ссылка «Источники фото», продолжение существующей страницы атрибуции.

Точный выбор дополнительных ингредиентов — редакционное решение в рамках запроса, отдельный внешний референс не заявляется. [Аудит и план](MENU_BOOK_COLOR_2026-09-13.md). Более ранние записи рисования ниже — история.

## Тонкий карандаш по скроллу — 2026-09-13

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Карандашные образы | Прямое «тонкой линией… как образы», текущие контуры блюд | Лёгкие линии формы и несколько внутренних деталей | Плотная серая штриховка, новые цветные блюда | SVG-штрихи по существующей геометрии |
| Переход к цвету | Прямое «резкий цветной переход, по скролле» | Прокрутка рисует линии, порог мгновенно включает фото | Таймер, растворение, движение продуктов | Прогресс от viewport и бинарный цвет |
| Книга и содержание | Выбранный разворот и последняя редакция | Прежние рецепты/приготовление, фотографии, мобильные листы | Новый шаблон раздела | Существующие компоненты и шрифты |

[Аудит и план](MENU_BOOK_SCROLL_LINES_2026-09-13.md). Точные толщины и дистанция прокрутки — параметры реализации прямого запроса; нового общего стиля нет.

## Карандаш, цвет, полноценное блюдо — 2026-09-13

Прямое уточнение отменяет перелёт компонентов: изображения рисуются и приобретают цвет на своих местах, готовая подача появляется целиком ниже. Осьминог сохраняется. Книга и действующая система остаются; отдельных декоративных компонентов не нужно. [Аудит и план](MENU_BOOK_PENCIL_COLOR_2026-09-13.md).

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Три фазы | Последнее уточнение пользователя и выбранный разворот | Карандаш → цвет ингредиентов → целая тарелка | Полёт, укладывание копий, изменение масштаба | Только раскрытие штрихов и opacity на месте |
| Утка | Прежняя одобренная duck-plate-960.webp | Реальная фактура и исходная диагональная подача | Повторная генерация цветного мяса | Оригинальное фото, его SVG-фрагмент и соответствующий карандашный слой |
| Каре | Кулинарный состав меню; [Chevanon Photography](https://www.pexels.com/photo/close-up-of-steak-meal-served-in-plate-323682/) | Естественные очертания отдельных рёбер и обжаренная фактура | Фон ресторана и композиция фотографии целиком | Новая иллюстрация по фотореференсу и её карандашная версия |
| Осьминог | Прямое «осьминог выглядит отлично» | Сохранить удачный предметный рисунок | Перерисовка и новая подача | Прежние файлы без изменения байтов |

## Выбранный разворот меню в основном сайте — 2026-09-13

Последующее «Добавь на очновеой» разрешает публикацию этого проверенного решения на существующем основном домене. Записи localhost ниже отражают этап интеграции. [Выпуск](RELEASE_MENU_BOOK_2026-09-13.md).

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Меню02 | Прямое «Внеси изменения на сайт»; animated-menu-book v2 | На одном листе ингредиенты/приготовление/сборка/тарелка, рядом другое блюдо | Самостоятельная шапка макета, старые шрифты | Нативный MenuBook в процессе |
| Широкая книга и маршрут | Выбранный двухстраничный макет, действующая сетка46/54 и40%/+55px | Читаемые два листа под рисунком01; следующие этапы ниже | Уменьшенный разворот в узкой колонке | Локальная адаптация сетки и измеряемой стрелки02→03 |
| Рисунки и фотографии | Шесть принятых atlas v2, наша утка и рецепт пользователя | Карандаш превращается в цвет, полная тарелка ниже | Новые подачи или выдуманные продукты | WebP-копии и прежняя SVG-обтравка |
| Текст и мобильное меню |12 описаний v2, последняя система Oranienbaum | Читаемое приготовление и один целый рецепт на телефоне | Шрифт Spectral из отдельной пробы | Действующие роли, кнопки≥44px |

[Аудит, план и итог проверки](MENU_BOOK_SITE_2026-09-13.md). Интеграция завершена; семь ширин и 200% текста проверены. Точная геометрия интеграции — адаптация выбранного макета. Новых стилистических решений без источника нет. Эта итерация — только localhost.
...
## Текущая шрифтовая система Oranienbaum — 2026-09-13

Эта запись заменяет шрифтовые роли предыдущих записей, сохраняя их как историю.

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Логотип, Hero, заголовки, названия частей блюда | [Прямое задание](../design/references/oranienbaum-brief-2026-09-13.md) | Oranienbaum 400 normal, минимум 19px, ink; точная шкала и нулевой трекинг заголовков | Старая система Spectral и Bellota, синтетические начертания | Общие токены и роли globals.css; наследование h1–h4; логотип 19px uppercase/+0.22em |
| Абзацы, интерфейс, курсив | То же задание | Ysabeau Office 400/500/600 и 400 italic, body 16.5px/1.62 | Новые тексты и отсутствующий FAQ | Настоящий курсив Hero; 600 только резервная роль вопроса |
| 01–04 и 25 | То же задание | PT Mono 400, tabular/lining; 22px/+0.04em для шагов | Отсутствующие цены, вместимость, время | Существующие station-number/numeric-spec, без изменения DOM |
| Сцена меню, коллаж, стикеры, маршрут, видео, contact/footer | Текущий утверждённый интерфейс и источники в карте ниже | Сохранение содержания, медиа, геометрии, палитры и поведения | Новые композиции, библиотеки, рукописные пометки и переименование | Только шрифтовые свойства; существующие названия блюда переводятся на display |
| Загрузка шрифтов | То же задание и официальные Google Fonts | Локальные WOFF2, два unicode-range, swap, два кириллических preload | Внешние запросы во время показа страницы и base64 | public/fonts и layout.tsx; проверка реальных Rendered Fonts |

Новых визуальных решений без источника нет. [Аудит, план и проверка](ORANIENBAUM_2026-09-13.md). Только localhost, домен не изменяется.

## Реальные ингредиенты над столом в02 — 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Визуальная часть02 | Прямой запрос; approved-menu-exploded-table-2026-09-13.png | Вертикальные ингредиенты над готовой тарелкой, выноски | Растровая еда из макета, лозунги и иконки | MenuExplodedDish с настоящими фото и прозрачностью |
| Стена и угол стола | Тот же снимок; палитра сайта | Светлая стена, перспектива стола, направление света | Чужие цвета, мраморная текстура, карточка | Локальный SVG |
| Подписи и mobile | Прежние названия частей блюда; референс и новое задание Oranienbaum | Короткие выноски, читаемые пояснения под узкой сценой | Растровый текст, дополнительный рукописный шрифт, анимация | HTML dt/dd в Oranienbaum/Ysabeau; при увеличении текста — пояснения одним столбцом |
| Реальные продукты и подача | Rachel Claire/Pexels, Madeline Liu/Unsplash, Kaboompics/Pexels, Safa Daneshvar и Miansari66/Wikimedia | Исходные фотографии, прозрачные маски по контуру | Генеративная еда, дорисовка скрытых частей, утверждение об авторстве шефа | WebP с alpha; оригиналы, маски и открытая страница авторства сохранены |

[Аудит, источники и проверка](MENU_EXPLODED_PHOTOS_2026-09-13.md). Остальные блоки и лесенка сохраняются. Только localhost; домен не изменяется. Точные координаты слоёв и масок — техническая адаптация снимка к существующей колонке, отдельного макета этих координат нет.

## Шеф и облако мысли в 02 — пробная отрисовка, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Шеф с блокнотом в «Продумываю меню» | Прямой запрос и действующий рисунок conversation-960.webp | Сохранить узнаваемого шефа и карандашную манеру | Гостья и весь сюжет знакомства | Один увеличенный растровый фрагмент 02 |
| Облако с будущим видео | Прямой запрос, графитные линии сайта, реальный фильм подготовки | Короткий штрих от головы к образу приготовления | Типовая карточка, выдуманный готовый ролик | Фотоэпизод внутри лёгкого контура мысли |

[Аудит, план и границы](../design/mockups/chef-menu-thought-2026-09-13/README.md). Новая геометрия — предложение; код сайта, адаптив и активные медиа не меняются.

## Стрелки между этапами — 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Соединения 01 → 02 → 03 → 04 | Прямой запрос других стрелок; существующие графитные сцены и проверенный маршрут процесса | Плавный карандашный штрих, открытые наконечники и проходы вокруг содержания | Новые иллюстрации, декоративные петли, перемещение этапов | Измеряемые SVG-кривые на desktop и вертикальные стрелки на mobile |

[Аудит и план](PROCESS_ARROWS_2026-09-13.md). Используются действующая палитра и одобренная композиция; решений без источника нет.
...
## Все три направления нарисованы — предложения, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Полный блок с раскрытой цветной подачей | Прямое «Нарисуй все3варианта»; свежий снимок графитных стрелок; идея пользователя о нажатии | Три рисунка и один цветной выбор | Старый пунктир, четыре инфографики, новые секции | Растровое состояние после нажатия |
| Полный блок с одним объёмным блюдом | Тот же запрос; пользовательский референс слоёв | Текстовый выбор категории, один управляемый объём | Бордовый фон, бургер, четыре параллельные3D-сцены | Растровое состояние разнесённой сборки |
| Единая сцена стола с этапами | Запрос нового подхода, действующие01–04 и графитные сценки | Показать вечер как изменение одной сцены | Постоянная сетка карточек, новые услуги | Основное состояние04 и отдельная раскадровка |

[Аудит, план, точные запросы и ограничения](../design/mockups/three-interactive-directions-2026-09-13/README.md). Все три изображения — неутверждённые предложения, не реализация или готовые3D-модели. Приложение сохраняется.

## Три идеи взаимодействия — предложения, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Нажатие на рисунок раскрывает фотографию | Прямая идея пользователя; существующие рисунки/фототарелка | Спокойное исходное состояние, один цветной акцент после действия | Постоянные четыре инфографики, новая тема | Отдельный разговорный интерактивный пример |
| Управляемая объёмная подача | Вопрос о3D и пользовательский референс слоёв | Один объект и его сборка по действию | Четыре тяжёлых параллельных сцены | Описанная концепция,3D-модель не создавалась |
| Стол от замысла к вечеру | Разрешение сменить подход, реальный смысл01–04 | Четыре этапа в одной меняющейся сцене | Новый набор услуг, случайный декор | Описанная альтернатива всему блоку |

[Аудит, план, источники и границы](../design/mockups/interactive-menu-ideas-2026-09-13/README.md). Код сайта не меняется. Рецептуры и экранный фильтр рисунка в демонстрации условные.

## Упрощение визуала02 — альтернативные макеты, 2026-09-13

Предыдущий полный вариант со слоями не выбран: пользователь считает его визуально неудачным и просит другие идеи. Он не становится утверждённым источником для реализации.

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| A: компактные готовые подачи | Прямой запрос улучшения, четыре категории, действующая фототарелка | Один крупный акцент и три малых в прежней зоне02 | Разборы, дубли готовых блюд,12 подписей, равная сетка | Полный растровый вариант без увеличения секции |
| B: набросок меню с одной настоящей подачей | Содержание этапа02, карандашные сцены01/04, действующая тарелка | План и результат в общем визуальном языке | Новый лист/карточка, иконки, фоновые эффекты | Четыре небольших этюда и одна цветная тарелка |

[Аудит, план, источники и ограничения](../design/mockups/calm-process-2026-09-13/README.md). Все новые координаты — предложения; приложение не изменяется.

## Весь блок с блюдами в разборе — только макет, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Все этапы01–04 с новой зоной02 | Прямое «нарисуй весь блок сайта»; актуальный artifacts/typography-roles-2026-09-13/process-1440.png | Существующая лесенка, текст, рисунки и видео;04 опускается по содержимому02 | Новый порядок, типовые карточки, увеличение всех элементов | Отдельный полный растровый макет |
| Четыре сборки внутри02 | Прямое продолжение предыдущего рисунка; пользовательский референс разнесённых слоёв | Вставить только две пары диаграмм в правую колонку | Повторный заголовок/текст02, новый глобальный стиль | Масштабирование четырёх сборок и увеличение высоты зоны медиа |

[Аудит, план и ограничения](../design/mockups/full-process-exploded-2026-09-13/README.md). Геометрия остаётся предложением; адаптив и приложение не меняются.

## Четыре блюда с разнесёнными слоями — только макет, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Стартеры, закуски, основное и десерт в разборе | Прямое «вот так, для всех4» и [снимок пользователя](../design/references/user-exploded-food-2026-09-13.png) | Каждый ингредиент отделён воздухом, слои соосны, внизу результат | Бургер, бордовый фон, реклама, деревянная поверхность | Отдельный растровый макет четырёх сборок двумя открытыми парами |
| Палитра, выноски, текстовые роли | Существующая бумага/латунь и актуальная система Spectral/Golos/IBM Plex Mono | Сохранить язык сайта, показать только фрагмент02 | Старый Bellota, новые компоненты, карточки | Макет с тем же заголовком и двумя абзацами |

[Аудит, план, запрос и ограничения](../design/mockups/exploded-four-courses-2026-09-13/README.md). Составы блюд условные; геометрия и адаптив не утверждены. Приложение не изменяется.

## Расположение меню и схема сборки — только макеты, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Подачи вдоль линии в этапе02 | Уточнение пользователя о другом расположении; утверждённый маршрут сайта | Последовательность четырёх курсов на общей бумаге | Карточки, сетка2×2, смешивание разных курсов в одно блюдо | Растровый вариант A «Как складывается меню» |
| Отдельные компоненты → готовая тарелка | Запрос схемы сборки; утверждённые четыре подписи MenuPlate, фраза о замысле блюда, карандашные сценки сайта | Показать четыре компонента отдельно и соединить с результатом | Типовые иконки, неподтверждённая рецептура как факт | Растровый вариант B «Как собирается блюдо» |

[Аудит, план, точные запросы и ограничения](../design/mockups/menu-schemes-2026-09-13/README.md). Геометрия схем и условные блюда предложены для обсуждения. Предыдущий растровый вариант не считается утверждённым источником стиля. Приложение не изменяется.

## Четыре категории еды — только макет, 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Зона тарелки в «Продумываю меню» | Прямой запрос пользователя о стартерах, закусках, основе и десерте; текущий artifacts/process-stagger-2026-09-13/process-1440.png | Четыре разные подачи на общей бумаге, прежние тонкие выноски и подписи; сохранение контекста этапов | Карточки, новый стиль, утверждение конкретных рецептов | Отдельный растровый макет, не подключённый к приложению |

[Аудит, план, источники и ограничения](../design/mockups/menu-courses-2026-09-13/README.md). Раскладка и блюда — предложение, не утверждённый референс. Активный интерфейс и его адаптивные правила сохраняются.

## Этапы ещё немного ниже — 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Этапы02/04 | Прямое «Если чуть чуть еще ниже?»; принятая лесенка30% | Небольшое дополнительное понижение в прежних колонках | Новые компоненты, оформление и mobile-раскладка | Коэффициент0.4 вместо0.3, прежние ограничения и пунктир |

[Аудит и проверка](PROCESS_STAGGER40_2026-09-13.md). Решений без источника нет; ниже сохранена история30%.

## Этапы лесенкой — 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| «От разговора — к вашему столу», этапы01–04 | Прямое «каждый подпункт шел ниже другого на30%»; существующий «План вечера» с Bellota Text | Каждый следующий этап ниже в существующих двух колонках; сдвиг02 и04 на30% высоты предыдущего содержания с защитой от пересечений | Новая сетка, карточки, тексты, медиа, декор | Измеренные отступы02/04 и адаптированная дуга; <=900px прежний вертикальный поток |

[Аудит, точная интерпретация и проверка](PROCESS_STAGGER_2026-09-13.md). Новых решений без источника нет.
...
## Пять шрифтовых макетов — 2026-09-13

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Сравнение пяти гарнитур | Прямое «С этими пятью 5 макетов»; последний подбор Balsamiq Sans/Pangolin/Bellota Text/Kurale/Lora; текущий первый экран | Одинаковый контент и композиция показывают различие настоящих шрифтов | Новый дизайн сайта, фото, тексты, утверждение новых токенов | Отдельные PNG desktop1440/mobile390 из изолированной браузерной страницы |

[Аудит, план и границы проверки](../design/mockups/five-fonts-2026-09-13/README.md). В действующем интерфейсе остаётся Shantell Sans.
...
## Мобильный макет «Фото и подпись» — 2026-09-11

Прямой запрос «Нарисуй макет» разрешает отдельный PNG обсуждённого улучшения. Это предложение, не внедрение и не одобрение новых токенов. [Аудит, файлы и ограничения](../design/mockups/formats-photo-captions-2026-09-11/README.md).

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Номер и название | Screenshot пользователя и действующий блок | Компактная строка освобождает ширину подписи | Карточки и вертикальная колонка номера | Предложение в PNG |
| Фотографии | Три действующих фото, одобренная общая область 3:2 | Сопоставимые полные снимки | Новые сюжеты и намеренная обрезка | Приблизительное растровое воспроизведение; точная геометрия нужна при вёрстке |
| Подпись под фото | Действующие тексты и задача связать изображение с описанием | Подпись рядом со своим изображением, общий левый край | Новые обещания | Предложенная перестановка только в PNG |
| Заголовок и разделители | Действующие Caveat, --gold-text и --rule | Смысловой акцент «ваш вечер», разделение форматов | Новая палитра и декоративные эффекты | Мобильный эскиз, точные размеры не утверждены |
...
## Частный дом в фоне первого снимка — 2026-09-11

Пользователь уточнил фон: частный дом, а не профессиональная кухня. Это заменяет цель стальных боковин. Сохраняется исходный передний план private-dinner.jpg (шеф, лицо, поза, руки и сковорода); фон вокруг него разрешено заменить и внутри прежнего кадра. Производная3:2 отдельно от оригинала. Источники — исходная фотография и прямой запрос домашнего окружения. [Аудит и промпт](../design/generated/private-dinner-home-2026-09-11/README.md). UI/публикация не меняются.

## Разрешена дорисовка бокового фона первого снимка — 2026-09-11

После требования сохранить те же фото пользователь прямо разрешил дорисовать фон по бокам первого снимка. Отдельная производная3:2 должна сохранять исходный кадр private-dinner.jpg по центру: лицо/позу/руки/сковороду без растяжения, новая только боковая область той же кухни. Никакой подмены hero-10. Визуальный источник — исходник; [план и промпт](../design/generated/private-dinner-outpaint-2026-09-11/README.md). Оригинал и интерфейс сохраняются; разрешение на дорисовку не является выбором или внедрением макета.

## Исходные фотографии обязательны — 2026-09-11

Прямое «Фото нужно оставить те же» отменяет предложение замены01 на hero-10. Использовать только исходные private-dinner.jpg, private-event-outdoor-crepes.png и masterclass.jpg; прежнее требование сохранения полного кадра действует. В исправленном отдельном макете рамки3:2 равны, фотоcontain/center; у портрета поля. Тонкая граница взята из текущего --rule и обозначает одинаковую область; не карточка услуги. HTML-макет загружает реальные файлы и локальный Caveat без imagegen. [Аудит и источники](../design/mockups/format-options-2026-09-11/ORIGINAL_PHOTOS.md). Код сайта прежний. Следующая запись — история отклонённого предложения.

## Одинаковый размер фото — продолжение исследования 2026-09-11

По предложению пользователя рисуется вариант с тремя фотографиями3:2. Для 01 предложен реальный горизонтальный hero-instagram/hero-10.webp1200×798, чтобы сохранить полный кадр без дорисовки вместо сильной обрезки старого портрета. 02/03, содержание, Caveat и палитра прежние. Источники — прямой запрос, текущая открытая композиция и пользовательская подборка реальных снимков. Замена фото исследуется только в макете и не считается утверждённой. [Аудит, план, промпт и границы](../design/mockups/format-options-2026-09-11/EQUAL_PHOTOS.md).

## Пять макетов форматов: исследование, без внедрения — 2026-09-11

Пользователь запросил пять рисунков улучшения блоков 01–03 по мобильному снимку. [Аудит, план, границы и результаты](../design/mockups/format-options-2026-09-11/README.md). Это предложения, не новая утверждённая версия интерфейса.

| Project element | Reference | Principle used | What is not copied | Project implementation |
|---|---|---|---|---|
| Компактные строки 01–03 | Действующая открытая таблица форматов; пользовательский снимок | Близость текста и соответствующего фото; сопоставимость альтернатив | Равные пустые 3:2-контейнеры, карточки | PNG 01, код прежний |
| Фото и заметка | Исходные пропорции трёх фото; текущая бумажная система | Текст занимает свободное поле у портрета, широкие сцены сохраняют контекст | Новые фото, декор блокнота | PNG 02, код прежний |
| Выбор по поводу | Реальные описания «для близких», «за разговором», «готовим вместе» | Смысл сценария перед названием услуги | Новые коммерческие обещания | PNG 03, код прежний |
| Переключаемый формат | Запрос логического улучшения; существующий список альтернатив | Постоянный индекс всех услуг + показ выбранного содержания | Готовый accordion, стандартные pills, маршрут последовательных шагов | PNG 04 с тремя состояниями; новый предлагаемый функциональный компонент, пока не утверждён |
| Фотооглавление | Документальные фотографии и одобренный принцип фотоколлажа; текущие номера | Общий визуальный обзор + компактный список с однозначной нумерацией | Декоративная фотостена, несвязанные снимки | PNG 05, код прежний |

Внешние референсы новых раскладок отсутствуют: предложения опираются на действующий дизайн, реальные сценарии и прямое поручение исследовать варианты. Imagegen интерпретирует фотографии и Caveat; внедрение должно использовать исходные медиа и локальные шрифты. Старые отклонённые макеты не являются источниками.
...
# Меню-разворот — отдельная рабочая проба, 2026-09-13

| Элемент | Источник | Принцип | Не переносится | Реализация |
|---|---|---|---|---|
| Бумажные страницы02 | Прямой запрос «реальная страница меню»; действующий paper и одобренная бумажная кромка | Один разворот с предметным сгибом, тонкими правилами и свободным полем | Повторяющиеся карточки, чужое ресторанное оформление | `design/mockups/animated-menu-book-2026-09-13/` |
| Разбор трёх блюд | Пользовательский exploded-food.png и перечисленный состав | Изолированные пищевые слои и осмысленные подписи | Бургер, красный фон, лишние продукты | Три иллюстративных фотокомпозиции imagegen с CSS-кадрированием |
| Контур → продукт → подача | Прямое пожелание анимации пользователя | Контур реального силуэта, раскрытие цвета и направленная сборка | Абстрактные3D-объекты, бесконечное декоративное движение | SVG-трассировка альфа и браузерная анимация; reduced motion |
| Типографика и внешнее поле02 | Текущий process-arrows/process-1440.png и утверждённые Spectral/Golos/IBM Plex Mono | Сохранить бумагу, графит и роли текста | Более ранние Bellota/Caveat и пунктир | Изолированные стили пробы, без правки app |
| Страницы на телефоне | Пользовательская метафора меню и необходимость читаемых продуктов/подписей | Одна страница за раз, доступное переключение | Уменьшенный настольный разворот | Отдельная адаптивная компоновка |

Подробный аудит и план до кода: [ANIMATED_MENU_BOOK_2026-09-13.md](ANIMATED_MENU_BOOK_2026-09-13.md). Гарнир каре — явно временное предложение; отсутствует подтверждённый рецепт. Макет не утверждает новую систему и не публикуется как действующий сайт.
## Одно блюдо на каждом листе меню — уточнение, 2026-09-13

| Элемент | Источник | Принцип | Не копируется | Реализация |
|---|---|---|---|---|
| Два соседних рецепта | Прямое «на одной странице и ингредиенты и сборка… на второй другое блюдо» | Завершённая история каждого блюда на одном листе | Прежнее разделение одного блюда на два листа | Обновление изолированного animated-menu-book |
| Красивая утка | Прямое «возьми утку нашу за основу», `public/media/web/duck-plate-960.webp` | Диагональная композиция, насечённая кожа, зелень, работа с соусом | Гарниры, не входящие в новый пример | Новая иллюстративная подача с сельдереем/морковью |
| Осьминог с зеленью | Прямой запрос и перечисленный ранее состав | Свежие зелёные акценты в продуманной ресторанной подаче | Случайный салат и посторонние компоненты | Новый фотографический atlas |
| Карандашные штрихи | Прямое уточнение вместо простого контура; карандашный язык сайта | Живой графит, объёмная штриховка, переход в соответствующее фото | Edge-detection как финальная иллюстрация | imagegen-рисунки по фотографическим исходникам + маска проявления |
| Действия с ингредиентами | Прямое «обязательно описание… осьминога жарим, оливки маринуем» | Пояснять приготовление и итоговую сборку | Цены, время/температуры, обещания | Короткие операции на обеих страницах |

[Аудит, план и проверка](MENU_BOOK_ONE_DISH_PER_PAGE_2026-09-13.md). Реализованы три полных листа и 12 описаний приготовления; проверены все семь ширин и анимация. Рецепт каре остаётся пробным. Уточнение относится к отдельному макету, а не к новым активным токенам сайта. Его типографика сохранена из исходной пробы; более новая система основного сайта не меняется. Технические SVG-маски управляют показом оригинальных PNG, не заменяя карандашные изображения.


## Гостья в «Знакомимся» — 2026-09-18

| Элемент | Референс | Принцип | Что не переносится | Реализация |
|---|---|---|---|---|
| Женщина справа в сцене разговора | Две фотографии пользователя от 18 сентября; существующая conversation-evgen-paper-flowers-v8.png | Узнаваемые черты лица и причёска в прежнем карандашном стиле | Фон фотографий, телефонный интерфейс, фотоколлаж; изменения шефа | Отдельный новый исходник и две responsive WebP-копии |

[Запрос, аудит и проверка](CONVERSATION_GUEST_2026-09-18.md). Компоновка и все остальные решения действующего сайта сохраняются.

## Видео подготовки — 2026-09-22

| Элемент | Референс | Принцип | Что не копируется | Реализация |
|---|---|---|---|---|
| Оформление ChefStoryVideo | Снимок пользователя 18:48:17; выбранный «Альбом шефа», CHEF_ALBUM_2026-09-18.md; действующая .format-photo | Живой кадр как фотография с бумажным полем, статическим наклоном и контактной тенью | Листы с дополнительным текстом, удалённая подпись, новые изображения/анимации | Изолированная оболочка прежнего видео 9:16 |

[Аудит и план](PREPARATION_FILM_2026-09-22.md). Видео, слова и другие разделы сохраняются.
