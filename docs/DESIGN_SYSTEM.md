## Размещение одобренного личного текста — 2026-09-13

Источник — прямое одобрение последней редакции «Кого соберём за столом?» до конца этапа «Ваш вечер», с исключением нового контактного блока и подвала. Шрифт Caveat, палитра --paper/--ink/--muted/--gold-text, коллаж, портрет, стикеры, маршрут и все медиа остаются. Hero получает имя/роль, h1 и вводный абзац в прежнем текстовом поле: существующая типографическая шкала и естественная высота вместо ограничения одним заголовком. Этапы поддерживают несколько обычных абзацев. Новых радиусов, теней, декоративных эффектов, библиотек или интерактивных состояний нет. Брейкпоинты 1100/900/620px сохраняются. Контакт/подвал/метаданные прежние. Итог: hero-identity использует прежний clamp(285px,31vw,435px) как min-height; портрет сохраняет тот же clamp как собственную высоту (на телефоне прежний aspect-ratio4/5 и height:auto). hero-copy — вертикальные текстовые роли, gap14px из существующего интервала подписей, прежние padding. Имя/вступление Caveat22px, на телефоне20px, line-height1.25/1.35; цвета --muted. H1 сохраняет прежнюю шкалу; «за столом?» использует прежний --gold-text, без отступа/nowrap от старого заголовка. Ширина мобильной панели сохранена после проверки читаемости. Высота первого ряда процесса и существующий измеряемый пунктир корректно адаптируются без изменения кода маршрута. Проверены семь обязательных ширин: текст/фото не обрезаны, пересечений нет.

[Аудит и план до кода](APPROVED_COPY_2026-09-13.md).
