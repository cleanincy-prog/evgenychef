...
Трёхсекундный сценарий и текущий mobile scale .42 сохраняются. Его CSS не зависит от scripting media query; no-JS и reduced motion сразу показывают статический Hero. Ожидание ведущих фото и шрифтов ограничено, позднего скрытия уже видимой карточки нет; изменение только высоты окна не отменяет вступление. [Аудит](HERO_IPHONE_2026-09-22.md).
