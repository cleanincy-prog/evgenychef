...
По прямому уточнению перед03 добавлено55px: desktop grid-template-rows = calc(max(прежний clamp, --conversation-row-height) +55px) auto. Коэффициенты02/04 остаются40%, пунктир измеряет итоговые узлы; прежний mobile<=900px не меняется. Другие токены и шрифтовые роли сохраняются. [Аудит и проверка](PROCESS_STEP03_2026-09-13.md).

## Этапы ещё немного ниже — 2026-09-13

По «Если чуть чуть еще ниже?» desktop-сдвиг02/04 увеличен30%→40%; CSS-fallback220/430px вместо165/375px. Проход56px, прежняя первая строка, форма пунктира, mobile<=900px и все остальные токены сохраняются. [Аудит и проверка](PROCESS_STAGGER40_2026-09-13.md). Следующая запись30% — предыдущая версия.

## Этапы лесенкой — 2026-09-13

Только в «От разговора — к вашему столу»: >900px сохраняются46%/54% колонки;02 получает верхний отступ30% высоты содержания01,04 — минимум30% высоты содержания03 с зазором56px под содержанием02. Начало03 — прежняя измеренная первая строка, минимум clamp(365px,31vw,440px). Отступы измеряются существующим компонентом маршрута; первая дуга учитывает разные высоты узлов. До900px прежний вертикальный поток36px и до620px31px между пунктами. Bellota400, палитра, изображения, прочие интервалы, состояния и библиотеки сохраняются. [Источник, аудит и проверка](PROCESS_STAGGER_2026-09-13.md).

## Выбранный Bellota Text — 2026-09-13

Прямое «Третий вариант» утверждает Bellota Text из пяти макетов. Теперь --font-pencil / --font-sans / --font-display используют Bellota Text normal400; это единственное фактическое начертание выбранного образца, включая прежние роли500. Кириллица/латиница локальные, WOFF2 inline/block, font-synthesis:none. Шкала размеров, line-height, палитра, сетки, интервалы, радиусы, рамки, тени, изображения, hover/focus, reduced motion и брейкпоинты сохраняются из действующей системы Shantell. Новых компонентов или библиотечной темы нет. [Утверждённый макет](../design/references/approved-bellota-text-desktop-2026-09-13.png), [аудит и проверка](BELLOTA_TEXT_2026-09-13.md). Нижележащие записи пяти проб и Shantell — история до выбора Bellota.

## Проба Shantell Sans — 2026-09-13

Пользователь выбрал Shantell Sans для всего сайта. Общие --font-pencil / --font-sans / --font-display переключаются на эту гарнитуру; normal 400 для текста и обычных заголовков, 500 для выразительных названий стикеров и кнопок. Кириллица и латиница хранятся локально и включаются в CSS как WOFF2 с font-display:block. Декоративные оси остаются в стандартном состоянии показанного образца; искусственный курсив/жирность отключены.

Палитра, контейнеры, сетки, интервалы секций, рамки, тени, изображения, действия, hover/focus, reduced motion и брейкпоинты прежние. Уточняются только размер/межстрочный интервал букв в существующих ролях. Проверенная шкала: body/биография 18px/1.45; описания 18px, до1100px17px/1.45; h2 clamp(32px,3.6vw,52px)/1.2, на телефоне clamp(29px,7.7vw,44px); Hero clamp(30px,3.2vw,46px)/1.2, до1100px clamp(28px,3.15vw,35px), до620px clamp(26px,7.2vw,42px). Названия стикеров clamp(26px,2.2vw,34px)/1.25, на телефоне25px/500; этапы clamp(24px,2.1vw,30px), планшет28px, телефон25px. Подписи тарелки: dt18–22px (телефон20px), dd16px (планшет17px)/1.4; видео16px/1.4. На901–1100px подпись «Текстуры» имеет ширину25%, абзац этапа03 получает padding-right12px. Header/footer допускают перенос ссылок при увеличении текста, сохраняя прежнюю высоту при100%; 200% на375/390px проверены без переполнения. [Аудит и план](SHANTELL_SANS_2026-09-13.md). Прежние записи Caveat ниже описывают версию до пробы.

## Позиция старта видео — 2026-09-13

По уточнению и снимку пользователя ролик начинает играть, когда весь кадр находится выше линии 94% видимой высоты страницы, оставляя место подписи и небольшой отступ снизу. В коротком окне, где кадр не помещается, запуск происходит при достижении верхом ролика верхней границы видимого окна. Достигнув позиции, ролик играет до полного ухода с экрана; ручная пауза, звук, reduced motion и прежняя ранняя загрузка сохранены. Новых CSS-токенов, декора, компонентов, состояний оформления и библиотек нет. [Источник, аудит и проверка](VIDEO_START_POSITION_2026-09-13.md).

## Загрузка Caveat без подмены — 2026-09-13

Действующие локальные Caveat 400–700, Unicode-диапазоны, метрики и все CSS-токены сохраняются. Шрифт включается в основной CSS как data URL, чтобы не ждать отдельного запроса после применения оформления. font-display:block исключает короткий показ резервной гарнитуры во время декодирования. Страница не скрывается JavaScript; без JavaScript шрифт также доступен. [Аудит и проверка](FONT_LOADING_2026-09-13.md).

## Одобренный текст с сохранением Hero — 2026-09-13

Последнее «В Хиро блоке текст не меняй, оставь как есть» исключает первый экран из текстовой переработки. Его заголовок, aria-label, структура и CSS возвращаются точно к a15887d: прежняя высота hero-identity, шрифтовые размеры, выравнивание и кадр портрета. Новые byline/intro и «Кого соберём…» удаляются. В одобренной редакции остаются биография под первым экраном, форматы, этапы и подписи. StationHeading поддерживает несколько отдельных абзацев. Caveat, палитра, сетки, брейкпоинты, стикеры, фото и состояния прежние; новых значений CSS/библиотек нет. Contact/footer/метаданные прежние.

[Источник, аудит и проверка](APPROVED_COPY_2026-09-13.md).

## Видео при прокрутке — 2026-09-12

Прямой запрос отменяет обязательное «Смотреть фильм»: видео постоянно в рамке 9:16 с contain, без скачка высоты. Существующие ширины/отступы по брейкпоинтам, бумага, Caveat и подпись сохранены. Встроенные средства управления — функциональный примитив браузера; кнопка «Повторить» в существующей бумажной стилистике появляется только при ошибке. Загрузка начинается до появления, тихое воспроизведение при входе в экран; ручная пауза и reduced motion сохраняются, вне экрана/скрытой вкладки видео приостанавливается. Новых декоративных токенов нет. [Аудит и проверка](VIDEO_SCROLL_2026-09-12.md).

## Верная фамилия — 2026-09-11

Пользователь уточнил: «Гребеник». Текущая идентичность — Евгений Гребеник / Evgen Grebenik. Изменяется содержание, а не визуальная система: прежние Caveat, размеры/веса, перенос фамилии в отдельном span, сетка, состояния, фото и стикеры сохраняются. В существующем OG-превью исправляется имя; неподтверждённая вместимость заменяется подтверждённым сайтом «Кипр». Новая social-preview концепция не создаётся. [Аудит и проверка](CHEF_SURNAME_2026-09-11.md).

## Выразительность бумажного стикера — 2026-09-11

По замечанию пользователя версия699c96b не передала глубину [утверждённого макета](../design/references/approved-paper-edge-sticker-2026-09-11.png). До корректировки повторно проверены CSS, SVG, текущие мобильные снимки и источник. План токенов: тонкая светлая кромка толщины бумаги рядом с контуром; более заметная протяжённая тень слева/снизу; локально плотнее Caveat у h3 и золотистое подчёркивание. Общий --paper, размеры фото/текста, поля, углы и адаптивность остаются. Финальные значения: h3 Caveat600 при прежнем размере; подчёркивание2.2px; контактная тень основы -1px 1px 1px rgb(92 70 35 / 16%); нижняя тень -4px 6px 8px rgb(92 70 35 / 32%) со слоя inset6px 8px 2px 2px/rotate(-.25deg). SVG-granь: теневой штрих #b4a999/.8/0.85px и непосредственно соседняя светлая кромка #fffdf7/.85/1.15px со смещением0.65/-0.65. Это цвет тонкой кромки, а не заливка бумаги. Проверены семь ширин и близость материала к макету. Слои декоративные, события не перехватывают, текст и фото не фильтруют; новых компонентов, библиотек и состояний нет.

[Аудит и проверка](PAPER_EDGE_EXPRESSION_2026-09-11.md). Ниже сохраняется история предыдущей слишком слабой реализации.

## Стикеры в цвет страницы с бумажным краем — 2026-09-11

Источник — [выбранный «Бумажный край»](../design/references/approved-paper-edge-sticker-2026-09-11.png). Заливка основы и окна фото теперь --paper #f4efe5. Небольшой SVG-контура использует существующий --rule #c8c0b3, тонкую линию и непериодические отклонения менее 1px при обычной ширине. Форма остаётся прямоугольной с мягкими углами. Светлая #fffdf7 и равномерная box-shadow прежней .format-row отменяются.

.format-row сохраняет ширину, padding10px/8px mobile, transform и раскладку. position:relative и isolation:isolate отделяют два декоративных слоя: ::before — бумага с контуром; ::after — короткая нижняя левая тень под ней. Неровность не действует на фото или текст. Тень использует прежний тёплый тон: -3px 6px 6px rgb(92 70 35 / 20%) на слое шириной57%, высотой28px, left4px/bottom3px и rotate(-1deg). Большая часть тени скрыта бумажным слоем; видимый остаток локален снизу слева. Форма основы border-radius6px 8px 5px 7px, stroke SVG0.85px, отклонения контура до0.55 единицы в viewBox600×500. Эти значения проверены на семи ширинах. Новых React-компонентов/библиотек/состояний нет. SVG — техническая геометрия утверждённого среза, без представительной иллюстрации. Hover/focus, текст, контраст, 3:2/contain, радиус фото3px, Caveat, кнопки, сетка, отступы и брейкпоинты прежние. Даты и номера услуг отсутствуют.

[Аудит, план, проверка](PAPER_EDGE_STICKERS_2026-09-11.md). Предыдущая запись ниже — история светлой оболочки.

## Цельные стикеры услуг — 2026-09-11

Основание: пользователь выбрал [цельные фото-стикеры](../design/references/approved-whole-stickers-2026-09-11.png) и поручил внедрение. Три альтернативы без нумерации и дат. Внутри одной бумажной оболочки: полное фото3:2, h3 и описание. Палитра текста/Caveat прежние; локальная бумага #fffdf7, радиус10px, фото3px, край10px (телефон8px), тень0 3px 10px rgba(92,70,35,.12) и0 1px 2px rgba(92,70,35,.06). Наклоны -1.4deg/+1.2deg/-1deg, на телефоне -1.2deg/+1deg/-.8deg. Это статическая форма из референса, а не анимация.

Desktop использует существующий контейнер90.5%/max1320px, локальные поля24px, промежуток44px, два стикера по половине ширины и третий центрирован ниже; второй сдвинут вниз на48px. До900px стикеры max560px следуют одной колонкой с разными выравниваниями. До620px поля списка6px и разрыв30px обеспечивают место для поворота без обрезки.

Название clamp(32px,2.8vw,42px)/1.12; на телефоне32px. Описание22px/1.35, до1100px20px/1.35. Подчёркивание h3 толщиной1.5px/отступ .18em использует --gold; общий заголовок2px/.18em. Бумажная подпись имеет внутренний отступ14px 12px 16px, на телефоне12px 8px 14px. Фотографии contain/center; размеры, alt, lazy/async и WebP прежние, sizes соответствует более крупным окнам.

У стикеров нет hover, кликов, focus, loading-состояний данных или анимации; ссылки/кнопки/видео и их доступность прежние. Reduced-motion сохраняет запрет анимации; пользовательская типографика должна оставаться читаемой при увеличении. Новых библиотек нет. Desktop-адаптация выведена из выбранной композиции, отдельного desktop-макета нет. [Аудит и проверки](FORMAT_STICKERS_2026-09-11.md).
