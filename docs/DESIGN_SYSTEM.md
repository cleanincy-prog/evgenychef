## Актуальное размытие коллажа: 6 px — 2026-09-22

По прямому запросу статический filter коллажа равен blur(6px), при running — none. Карточка и портрет не фильтруются; остальные правила прежние.

## Актуальное размытие коллажа: 8 px — 2026-09-22

По «Еще больше размытости» статический filter коллажа равен blur(8px). Во время running сохраняется none. Карточка и портрет не фильтруются; остальные токены и правила прежние.

## Актуальное размытие коллажа: 5 px — 2026-09-22

По «Еще больше размытия» готовый коллаж использует blur(5px). Во время running остаётся filter:none, прочие токены и стили прежние. [Аудит](HERO_COLLAGE_BLUR_5PX_2026-09-22.md).

## Размытие готового Hero — 2026-09-22

Актуальный blur коллажа: 3 px вместо 2 px по прямому запросу. В running остаётся filter:none для плавности. Портрет, текст, изображения, сетка, палитра и шрифты прежние. [Аудит](HERO_COLLAGE_BLUR_2026-09-22.md).

## Совместимый запуск Hero — 2026-09-22

Трёхсекундный сценарий и текущий mobile scale .42 сохраняются. Его CSS не зависит от scripting media query; no-JS и reduced motion сразу показывают статический Hero. Ожидание ведущих фото и шрифтов ограничено, для предварительного decode коллажа предусмотрено до 800 ms, позднего скрытия уже видимой карточки нет; изменение только высоты окна не отменяет вступление. [Аудит](HERO_IPHONE_2026-09-22.md).

## Полная сцена знакомства — 2026-09-22

Внешний кадр 1280×1080 объединяет прежний ролик 1280×720 сверху и согласованное неподвижное продолжение. Прежний масштаб, графитный фильтр, multiply, палитра и адаптивная ширина сохраняются; высота растёт по пропорции. Лица, руки, предметы и реплика берутся из прежнего видео. Низ виден также при reduced motion/no-JS/ошибке; два проигрывания и прочие правила сохранены. [Аудит](CONVERSATION_SCENE_2026-09-22.md).

## Движение Hero — 2026-09-22

Четыре ключевых кадра справа → полёт влево → ускорение → плавный портрет и текст; 3000 ms. Обычные CSS transform/opacity, без вложенного blur во время движения. Конечная сетка/blur 2px/шрифты/paper/ink прежние. Reduced motion и ошибки загрузки оставляют статический первый экран. [Проверки](HERO_RELEASE_2026-09-22.md).

## Заголовок опыта — 2026-09-22

Актуальный текст: «Больше 20 лет работы шеф-поваром». Сохраняются award-title, Oranienbaum и numeric-spec у числа 20; размеры и адаптивная раскладка прежние. [Аудит](EXPERIENCE_HEADING_2026-09-22.md).

## Этапы с последовательным понижением — 2026-09-22

На ширинах от 901 px правые этапы начинаются на 60% высоты содержания предыдущего левого этапа ниже; стартовые значения без измерения — 17.5rem и 25rem. Сохраняются колонки 46%/54%, проходы маршрута, полная ширина тарелки, палитра, Oranienbaum / Ysabeau Office / PT Mono и существующие медиа. До 900 px прежний вертикальный поток без дополнительных сдвигов. [Основание и проверки](PROCESS_STAGGER_2026-09-22.md).

## Движение видео знакомства — 2026-09-22

Предзагрузка за 2000 px до кадра; начало при 20% видимой площади, после старта продолжение пока кадр виден. Ровно два полных проигрывания, затем неподвижный последний кадр до перезагрузки страницы; уход/возврат не сбрасывает счётчик. Прозрачный слой защищает от касаний, сохраняя прокрутку и масштабирование страницы. Мute/inline, без controls; reduced motion и no-JS сохраняют постер. Размеры, графитный фильтр, палитра, шрифты, интервалы и остальные видео прежние. [Аудит](CONVERSATION_PLAYBACK_2026-09-22.md).

## Продукты и этапы без номеров — 2026-09-22

В ChefSourcing отсутствует колонка 01–03; тексты занимают всю прежнюю строку. StationHeading сохраняет узел маршрута и текст в двух колонках; прежняя колонка 01–04 удалена на всех брейкпоинтах. Содержательные числа, палитра, типографика, разделительные линии, изображения и видео сохраняются. Источник — прямое поручение пользователя; новых визуальных решений нет. [Аудит](NUMBERING_REMOVAL_2026-09-22.md).

## Текст об авторской кухне — 2026-09-22

Меняется только содержимое award-note по точному тексту пользователя. Прежние Ysabeau Office, размер, цвет, отступы, колонка и мобильная полная ширина сохраняются. Новых токенов или визуальных компонентов нет. [Аудит и проверки](BIOGRAPHY_COPY_2026-09-22.md).

## Плавное немое видео тетради — 2026-09-22

MenuWorktable сохраняет ширину 38.75rem, поля и прежний grayscale/contrast/multiply. Внутренний кадр 16:9 соответствует пользовательскому ролику и показывается целиком. MP4 без аудио, 60 fps, темп 0.8×; запуск по видимости и повтор через прежний usePageMediaPlayback. Reduced motion/no-JS — постер первого кадра; ошибка — постер и существующая кнопка с целью не менее 44 px. Остальные блоки и токены прежние. [Аудит](MENU_VIDEO_2026-09-22.md).

## Немое видео знакомства — 2026-09-22

Сохраняется существующая ширина и мобильные поля conversation-illustration. Пользовательское видео имеет собственные пропорции 1280:720: полный кадр, object-fit:contain, без обрезки лиц и текста. Сохраняется карандашное смешивание с фоном process-illustration. Панели плеера нет; при ошибке показываются постер и существующая кнопка «Повторить». Беззвучное воспроизведение управляется usePageMediaPlayback; системное уменьшение движения и no-JS оставляют постер. Палитра, шрифты, размеры остальных блоков прежние. [Аудит](CONVERSATION_VIDEO_2026-09-22.md).

## Hero без строки услуг — 2026-09-22

В `.hero-copy` остаётся только прежний h1 «Евгений Гребеник — ваш частный Мастер-Шеф на Кипре». Абзац услуг и его отдельное CSS-правило удалены без пустого места под строкой. Полноэкранная высота, шрифты, портрет, коллаж и адаптивные правила сохраняются.

## Заголовок DishNotes — 2026-09-20

Текст h3 теперь «Как формируется вкус» по прямой редакции пользователя. Роль Oranienbaum, размеры, переносы, отступы и весь восстановленный блок прежние.

## Прежний DishNotes — 2026-09-20

Полностью возвращается одобренный блок «Как складывается вкус» из выпуска 95: Oranienbaum / Ysabeau Office, paper/ink, кремовая тарелка, четыре HTML-подписи и три тонкие выноски. На телефоне две подписи над тарелкой и две под ней; desktop и увеличение текста используют те же прежние правила. CSS, шрифты и фото уже побайтно совпадают с этой версией. Остальные блоки сохраняют выпуск 96. [Аудит](RESTORE_DISH_BLOCK_2026-09-20.md).

## Финальная редакция и строка услуг в Hero — 2026-09-20

Сохраняются Oranienbaum / Ysabeau Office / PT Mono, paper/ink, коллаж, портрет и полноэкранная высота. Текстовая область Hero получает вертикальную раскладку: существующий h1 и краткий абзац услуг в роли основного текста, без фиксированной высоты и обрезки при увеличении шрифта. Числа 25 и 20 в заголовке опыта используют PT Mono. Для новой строки «ваш частный» убран внутренний сдвиг курсивной строки, чтобы сохранить пять строк при обычном размере. Исходное фото содержит чёрные поля: минимальная высота изображения 170% сохраняет фотографическую часть в ставшем выше планшетном кадре. При контейнере до 20rem текст Hero занимает почти всю бумажную область, а межстрочный интервал h1 составляет 1.2; это помогает при увеличении текста. Отступ под удалённым kicker выбора продуктов не сохраняется. [Текст и план](FINAL_COPY_2026-09-20.md).

## Тетрадь с расчётом на 12 человек — 2026-09-20

Меняется только содержание надписей внутри рисунка MenuWorktable. Сохраняются 3:2, масштаб и отступы, graphite/multiply, существующие загрузка/ошибка/повтор и мобильный цельный кадр. Все HTML-шрифты, цвета, размеры и внешний текст прежние. Рукописные записи соответствуют утке в соседнем DishNotes; числа помечены как пример. [Аудит](DUCK_NOTEBOOK_2026-09-20.md).

## Удаление двух служебных подписей — 2026-09-19

По прямому запросу убраны подпись под тарелкой и заключительная фраза sourcing-раздела. Освободившиеся строки удаляются из сетки. Все действующие шрифты, цвета, изображения и интервалы остального содержимого сохраняются. [Аудит](CAPTION_CLEANUP_2026-09-19.md).

## Подписи к выбранному блюду — 2026-09-19

Используются прежние paper #f4efe5, ink #0a0a0a, muted #6e665a, Oranienbaum для заголовка и Ysabeau Office для четырёх коротких пояснений. Из одобренного макета сохраняются крупная кремовая тарелка и три тонкие выноски с точками. Подписи остаются HTML, линии декоративные. Нет карточек, рамок, новых теней и анимаций. На узком контейнере две заметки сверху и две снизу; при увеличении текста — последовательное чтение без обрезки. Фото и автор указаны отдельной небольшой ссылкой. [Аудит](DISH_NOTES_2026-09-19.md).

## Компактная секция выбора продуктов — 2026-09-19

По прямому замечанию уменьшаются только размеры и внутренние интервалы секции: desktop-композиция до 63rem с портретом до 23rem; на телефоне портрет и введение рядом, продуктовые строки на всю ширину. Шрифты, палитра, линии и содержание прежние. Мобильные подписи от 1.0625rem; в особенно узком контейнере или при увеличенном тексте — одна колонка. [Аудит](CHEF_SOURCING_COMPACT_2026-09-19.md).

## Новый раздел «Почерк шефа» — 2026-09-19

Применяется точная бумага/тушь, три действующие гарнитуры и композиция выбранного макета 6. Ширина раздела — общий контейнер 82.5rem; поля и ограничения рисунка переведены в rem для принятого масштаба 16–24 px. Портрет показывается целиком с прежним графитным фильтром примерки. h2 раздела в Oranienbaum, h3 продуктов той же гарнитурой; основной текст Ysabeau Office, номера PT Mono. Открытые строки разделены тонкой линией --rule, без новых теней/скруглений. При ширине контейнера до 44rem — одна колонка; на телефоне сохранены крупные изображение и подписи. Изображение имеет явное соотношение 2:3. Новых состояний интерактивности нет. [Аудит](CHEF_SOURCING_2026-09-19.md).

## Пропорции и широкий экран — уточнение 16:08, 2026-09-18

По новому замечанию неподвижный масштаб 1320 px заменяется относительным: root `clamp(100%, 1.111111vw, 150%)`, общий контейнер 82.5rem. До 1440 px базовая единица остаётся 16 px, на широком экране достигает 24 px; настройки шрифта пользователя учитываются через процентные границы. Активные поля и максимумы масштабируются в rem. Заголовки разделов до 3.25rem; форматов 2–2.5rem с плавной промежуточной величиной, описания 1.0625–1.25rem, вводные 1.1875–1.5rem. Лист центрируется относительно фото, поля 1.75/2/1.875rem; нет искусственного растягивания короткого описания. Фраза об опыте становится h2. Остальные роли, фото, стили бумаги, состояния и мобильная последовательность сохраняются. [Аудит](ALBUM_PROPORTIONS_2026-09-18.md).

## Первый экран и ритм страницы — 2026-09-18

Hero: минимум `100svh` минус действующая шапка 80/72/74 px без верхнего ограничения; текст может увеличить высоту. Общая внешняя ширина 90.5%, максимум 1320 px, поля телефона 18 px. Опыт шефа, альбом и процесс используют общие границы. Интервал основных разделов `clamp(48px,6vw,88px)`, заголовка и содержимого `clamp(32px,4vw,56px)`; интервал фотографий альбома 56 px на desktop и прежние 42 px на телефоне. У опыта только горизонтальные линии прежнего rule, внутренние вертикальные поля 24–40 px и длина текста до 44rem. Палитра, семейства/роли шрифтов, состояния ссылок, движение, портрет и фото сохраняются. Мобильный альбом до 30rem выравнивает по себе заголовок. [Аудит](HERO_PAGE_FLOW_2026-09-18.md).

## Выбранный «Альбом шефа» — 2026-09-18

Для трёх форматов применяется прямо выбранный [макет](../design/references/chef-album-approved-2026-09-18.html): прежний paper `#f4efe5`, локальный светлый лист `#faf7f0`, ink `#0a0a0a`, текст листа `#514b41`, золотистый штрих `#a0792e`; Oranienbaum 400 32 px, Ysabeau Office 400 17 px / 1.49 и вводная фраза 19 px / 1.35. Размеры реализуются в rem. Альбом до 1000 px, 12 долей, фото в 7 долях, лист в 5 долях с нахлёстом 24 px и отступом сверху 50 px. Поле фото 10/10/20 px, лист 25/26/27 px; прямые бумажные края и слабые локальные тени соответствуют макету. Статические наклоны фото −2°/+1.8°/−1°, листов ±0.65°. До 760 px — отдельная последовательная компоновка фото/листа; лист перекрывает нижнее бумажное поле на 10 px, текст не имеет фиксированной высоты. При увеличении текста контейнер до 42rem включает ту же просторную последовательную раскладку; длинные русские заголовки переносятся через hyphens:auto. Новых hover/focus-состояний, контролов и анимаций нет. Действующие ссылки, доступность, остальная палитра и загрузка шрифтов сохраняются. [Аудит](CHEF_ALBUM_2026-09-18.md).

## Предпросмотр ссылки — 2026-09-18

Прямой запрос заменяет старое оформление OG и значка. Применяется текущая система Oranienbaum 400 / Ysabeau Office 400 и paper #f4efe5, ink #0a0a0a, gold-text #72561f. Обложка использует имя, короткую услугу и портрет из Hero; прямоугольный фотокадр без теней, декоративных диагоналей и скругления. Монограмма ЕГ наследует настоящий Oranienbaum. Размеры/сетки/брейкпоинты и состояния основного сайта не меняются. Новые тексты метаданных описывают реальные услуги на Кипре. [Аудит и проверка](LINK_PREVIEW_2026-09-18.md).

## Видео без панели управления — 2026-09-15

Прямой запрос убирает из видео stop/pause, перемотку и прочие штатные элементы плеера. Сохраняются film-stage 9:16, постер, размеры и отступы. Беззвучное inline-видео повторяется, запускается в прежней позиции прокрутки и приостанавливается вне кадра; reduced motion учитывается. Кнопка повторной загрузки остаётся только при ошибке. [Аудит и проверка](QA_FIXES_VIDEO_2026-09-15.md).

## Новая редакция блока об опыте — 2026-09-15

Заголовок award-title получает пользовательскую формулировку про 20 лет работы шеф-поваром; число сохраняет numeric-spec. Четыре смысловых абзаца используют прежний award-note. Фото, цвета, размеры, отступы, настольный flex и мобильная сетка прежние; новые стили и токены не вводятся. [Аудит и текст](ABOUT_CHEF_COPY_2026-09-15.md).

## Мастер-классы и короткий этап 04 — 2026-09-15

Новая пользовательская редакция использует прежний абзац описания формата и два прежних текстовых элемента этапа «Ваш вечер». Содержание сокращается без пустых контейнеров; иллюстрация и маршрут сохраняются в существующем потоке. Новых стилей нет. [Аудит](MASTERCLASS_EVENING_COPY_2026-09-15.md).

## Редакция описания частного ужина — 2026-09-15

Новый пользовательский текст про авторский ужин в 7 подач использует прежнее оформление описания формата. Размеры, гарнитуры, фото, отступы и сетка не меняются. [Аудит](PRIVATE_DINNER_COPY_2026-09-15.md).

## Контакт без подсказки — 2026-09-15

По прямому запросу удалены contact-help и его отступы; контакт состоит из прежней кнопки Instagram и аккаунта. Новых стилей нет. [Аудит](CONTENT_CLEANUP_2026-09-15.md).

## Стабильность первого показа Hero — 2026-09-15

Действующие Oranienbaum 400, Ysabeau Office 400 normal/italic и PT Mono 400 доставляются внутри основного CSS с font-display:block. Те же data URL предварительно декодируются через ReactDOM preload в серверном layout. Отдельных сетевых запросов используемых начертаний и JS-экрана ожидания нет. Портрет сохраняет исходное соотношение 576/1280 до и после загрузки responsive-варианта; кадрирование −21% прежнее. Роли, размеры, пять строк Hero, фото и сетка сохраняются. Высоту текста нельзя жёстко фиксировать ради устранения сдвигов: увеличение текста остаётся доступным. [Аудит и проверка](HERO_LOADING_2026-09-15.md).

## Удаление условий и примера меню — 2026-09-15

Прямой запрос удаляет время/место форматов, обычную подпись видео, отдельный sample-menu с блюдами и заголовок контакта. Их стили и пустые контейнеры удаляются. Контакт на всех ширинах использует уже существующую одноколоночную схему: прежняя кнопка, аккаунт, подсказка, ширина до 510 px; вертикальный разделитель двух колонок больше не нужен. Ошибка видео появляется только при сбое. Палитра, шрифты, изображения, бумажные рамки, этапы и остальные отступы сохраняются. [Аудит и проверка](CONTENT_CLEANUP_2026-09-15.md).

## Удаление FAQ и тарифов — 2026-09-15

По прямому уточнению FAQ и цены/количество гостей удалены. У форматов остаются время и место; оба поля занимают две колонки прежней сетки. Шрифты, цвета, изображения, масштаб и бумажная рамка прежние. Фотография мастер-класса использует JPEG 384/768 из того же исходного файла для корректной выдачи image/jpeg. Оформление удалённого FAQ больше не применяется. [Аудит и проверка](FORMAT_CLEANUP_2026-09-15.md).

## Меню, форматы и FAQ — 2026-09-14

Прямое задание menu-formats-faq-brief-2026-09-14 заменяет прежний вес UI 500 на 400: единственный вес выше 400 на странице — вопросы FAQ 600. Oranienbaum 400 normal, ink, трекинг 0, минимум 19 px. Меню: имя 20 px/1.3 (mobile 19 px), сопровождение 15 px/1.5 (mobile 14.5 px), номер PT Mono 13 px/1.5, tracking .02em, tabular/lining. Строки с dotted rule, padding 10 px, gap 14 px. Форматы: имя 20 px/1.3, верхний rule, padding 11 px, margin 14 px; спецификации PT Mono 13 px/1.5, gap 4×18 px, цена последняя gold-text; одинаковые колонки обеспечивают общую вертикаль. FAQ: вопрос Ysabeau 600 18 px/1.35, ответ 400 16.5 px/1.62, до 52ch; на <=760 px line-height 1.70. На этой же границе сопровождение меню отдельно без первой точки, спецификации 2×2 с gap 6×14 px. Значения размеров записываются в rem. Палитра, фото-стикеры, графит, контакт, состояния фокуса и reduced motion наследуются из действующего сайта. Новые тени, радиусы или анимации не вводятся.

[Аудит и проверки](MENU_FORMATS_FAQ_2026-09-14.md).

## Интеграция выбранного графитного шефа — 2026-09-14

По прямому разрешению макет переносится на сайт: одна сцена 1536×1024, до 620 px, 17 px сверху и 34 px слева внутри колонки 02. Фон #f4efe5 и текущие шрифты сохранены; рисунок получает прежние multiply / grayscale(1) contrast(1.25). На телефоне он целиком умещается в ширину; раскрытия и цветного фотослоя нет. Сохраняются сдвиг этапов 40%, проход 55 px и исходные тексты. Новых интерфейсных токенов или библиотек нет. [План и проверка](MENU_CHEF_NOTES_SITE_2026-09-14.md).

## Тексты вне иллюстрации сохраняются — уточнение 2026-09-14

По прямому уточнению меняются слова на бумагах в руках шефа. Заголовок «Продумываю меню», исходный абзац и все действующие стили сохраняются. Последняя расширенная редакция внешнего текста не является выбранной. Крупный вид тетради — отдельный материал для просмотра; он не добавляет интерфейс на сайт. [Аудит](../design/mockups/chef-menu-copy-2026-09-14/README.md).

## Новые надписи в прежнем рисунке — 2026-09-14

Меняется текст отдельного макета: короткие рабочие формулировки на бумагах и новая редакция этапа. Одобренные ракурс, графит, почерк, #f4efe5 и текущие шрифты сохраняются; фотография тарелки не возвращается. Числа остаются проверяемым примером, неизвестные поля — незаполненными. [Редакция и аудит](../design/mockups/chef-menu-copy-2026-09-14/README.md). Новых визуальных токенов и активных компонентов нет.

## Макет без цветной тарелки — 2026-09-14

По прямому запросу убрать тарелку сцена становится полностью графитной: человек, тетрадь и рабочие листы. Существующие фон, шрифты, масштаб и расположение рисунка сохраняются; новых активных токенов нет. Текст уточняется отдельно. [Аудит](../design/mockups/chef-menu-no-plate-2026-09-14/README.md).

## Реальные записи вместо декоративных схем — 2026-09-14

В отдельном макете сохраняются выбранный ракурс через плечо, графит, #f4efe5, действующие шрифты и отдельная фотография рыбы. Содержание страниц строится по реальным тетрадям и рабочим формам. Количества берутся из атрибутированного опубликованного примера; неизвестные остатки и закупочное брутто не заполняются. Большие презентационные схемы, вымышленные граммовки и размеры тарелок исключаются. [Источники и аудит](../design/mockups/chef-menu-real-notes-2026-09-14/README.md). Новых активных токенов или компонентов нет.

## Выбранный взгляд через плечо: подробная разработка — 2026-09-14

В новом эскизе сохраняются ракурс третьего варианта, #f4efe5, текущие шрифты и тонкий графит. По прямому запросу увеличивается количество осмысленных рабочих материалов: четыре разных листа, эскизы подачи, расчёт порции, сочетания и порядок блюд, исправления и видимые края прежних страниц. Иерархия остаётся у руки и тетради, листы естественно перекрываются. Фото рыбы сдвинуто на передний правый край, остаётся цветным отдельным исходным слоем. Это [доработка макета](../design/mockups/chef-menu-studies-2026-09-14/README.md), без новых активных токенов, интерфейсных карточек или анимации.

## Лист меню в руках — дополнительный эскиз, 2026-09-14

Вариант 1 использует те же бумагу #f4efe5, тонкий графит, реальные Oranienbaum / Ysabeau Office / PT Mono и оформление этапа. Различается сюжет: обычный лист у груди, читающий взгляд, карандаш у строки, небольшой закрытый блокнот. Фотография рыбы остаётся отдельной и цветной. Это [не выбранная пользователем примерка](../design/mockups/chef-menu-options-2026-09-14/README-option-1.md), активные токены и компоненты не меняются.

## Выбор блюд / взгляд через плечо — новые эскизы, 2026-09-14

Заказаны два разных сюжета на общей бумаге #f4efe5: отбор отдельных листков и работа со стороны плеча шефа. Масштаб бумаги естественный относительно рук. Тонкий графит и тёмные акценты фартука — из одобренного разговора. В третьем варианте цветная рыба является отдельным исходным фотослоем; не получает монохромного фильтра. Существующие шрифты и оформление этапа сохраняются в отдельных примерках. Новых активных токенов, компонентов или анимаций нет. [Источники и план](../design/mockups/chef-menu-options-2026-09-14/README.md).

## Понятное составление меню — уточнение примерки, 2026-09-14

Последующий запрос отклоняет эту примерку. Большая тетрадь перед шефом не является выбранным композиционным правилом; поиск продолжается без изменения активной дизайн-системы.

Замечание пользователя меняет содержание рисунка: большой раскрытый разворот с «Меню вечера», названиями блюд и эскизами подач. Шеф касается эскиза карандашом, рядом отдельная настоящая фотография выбранной рыбы. Бумага #f4efe5, графит и существующая типографика сохраняются. Цветной фотослой не получает графитного фильтра. Новых токенов, анимаций, карточек и активных стилей нет. [Аудит и границы](../design/mockups/chef-menu-planning-2026-09-14/README.md).

## Шеф с тетрадью — примерка, 2026-09-14

Прямой запрос макета использует действующие paper #f4efe5, muted #6e665a, Oranienbaum / Ysabeau Office / PT Mono и оформление этапов. Новый сюжет выполняется графитом в манере одобренного разговора с шефом. Никаких новых активных токенов, рамок, карточек или анимации. Это отдельная примерка, активный сайт прежний. [Источники и план](../design/mockups/chef-purchasing-2026-09-14/README.md).

## Подложка выбранного макета — 2026-09-14

Пользователь отклонил предыдущую фотографическую композицию. Текущая подготовка возвращает именно выбранный макет с пустой передней тарелкой: его кремовый свет, альбом под углом, бумага, рисунки и реквизит сохраняются. Новые CSS, гарнитуры и сетка не вводятся. [Результат и границы правки](../design/mockups/menu-approved-empty-plate-2026-09-14/README.md). Эта отдельная подложка пока не заменяет активные файлы сайта.

## Фотографический рабочий стол — 2026-09-14

Один общий кадр с настоящей прошитой тетрадью, тарелкой и карандашом на paper #f4efe5. Фактура, сгиб и блики из фотографий; тени только предметные. Без градиентной маски поверх предметов, UI-карточек и анимации. Camera сверху согласует реальные источники, отдельного горизонта стены нет. Главная рыба сохраняет исходный RGB.

Рукописный Caveat локальными WOFF2 только внутри тетради по утверждённому эскизу рабочего стола. Текст страниц масштабируется вместе с кадром, две страницы остаются рядом на телефоне по прямому запросу. Внешние название и подпись, ссылки, focus/hover/error/retry, сетка процесса и остальной сайт сохраняют Oranienbaum / Ysabeau Office / PT Mono и текущие размеры. Ширина до1120px, проход маршрута48/38/33px и breakpoints900/620px прежние. Ни раскрытия, ни горизонтального скролла. [Аудит](MENU_PHOTOGRAPHIC_2026-09-14.md).

## Цельная сцена на всех ширинах — 2026-09-14

Прямой запрос отменяет последовательные страницы, отдельную тарелку, раскрытие и увеличение. Единый кадр тетради с блюдом сохраняет пропорции1419:1108 и текущую ширину контейнера. Надписи и фотографии внутри кадра масштабируются с ним; это иллюстративные детали. Название и подпись снаружи кадра используют действующие Oranienbaum/Ysabeau Office и полноценный размер. Цвета, кадр, настоящие фотослои, внешние отступы, состояния, фокус и отсутствие анимации прежние. Не добавляются диалог, управление увеличением, обрезка через overflow:hidden или горизонтальная прокрутка. [Аудит](MENU_NOTEBOOK_ONE_BLOCK_2026-09-14.md).

## Разворот ингредиентов — 2026-09-14

Новый запрос заменяет содержание двух страниц и посуду. Фон блока точно `--paper:#f4efe5`; стена, стол и глубокая матовая кремовая тарелка находятся в предметной подложке. Действующие Oranienbaum/Ysabeau Office/PT Mono используются для живого текста поверх страниц. Левая страница — описание и состав рыбы с овощами, правая — шесть настоящих фотографий компонентов. Еда сохраняется отдельным исходным фотослоем без ретуши. Сцена расширена до1120px ради читаемого разворота; основной текст16.5px. До62rem ширины контейнера страницы получают обычную высоту по содержимому и фактуру из той же подложки, тарелка показывается ниже. До40rem — последовательные страницы, до18rem — одна колонка фото, включая увеличение текста. Контактные тени принадлежат изображению; края подложки мягко совмещены с paper. Нет новой карточной оболочки или анимации. Остальные токены и маршрут прежние. [Аудит и проверка](MENU_WORKTABLE_INGREDIENTS_2026-09-14.md).

## Выбранное фото рыбы — 2026-09-14

В существующем рабочем столе меняется только подача и согласованные надписи/набросок. Цветной слой — присланная фотография, без цветовых фильтров, смешивания с бумагой и синтетических продуктов. Разрешены контурное отделение тарелки и отдельная контактная тень из одобренной предметной композиции. Полный кадр 3:2, до 960 px, мобильная ширина, палитра, типографика, состояния и маршрут прежние. [Аудит](MENU_WORKTABLE_FISH_2026-09-14.md).

## Примерка новой фотографии утки — 2026-09-13

Прежний монтаж отклонён. Новое предложение BOKA проверяется отдельно: матовая светлая тарелка, читаемый объём еды, мягкий свет и масштаб из одобренного рабочего стола. Это параметры исследования, не новые токены сайта. Нейтральная страница просмотра использует текущие бумагу, чернила и шрифтовые роли, без карточек и анимации. Все активные стили остаются прежними. [Аудит](../design/references/duck-worktable-selection-2026-09-13/README.md).

## Карандашная история создания меню — 2026-09-13

**Локальная интеграция рабочего стола:** полный кадр 3:2 до 960 px, без CSS-фильтров, наложения цвета, рамки, скруглений или добавленной тени. До 900 px — доступная ширина с прежним левым проходом для маршрута (38 px, до 620 px — 33 px). Подпись примера меню — существующая Ysabeau Office 16.5 px, ссылка источника — существующая вторичная роль; focus/hover и 44 px сохраняются. При 200% текста подпись свободно переносится. Остальные токены и 40%/+55 px прежние. [План](MENU_WORKTABLE_SITE_2026-09-13.md).

Во время проверки 200% обнаружен перенос цифр номера этапа. Номера получают nowrap; вторая колонка заголовка использует minmax(прежняя ширина, max-content), чтобы при увеличении текста вмещать две цифры. При 100% размеры и композиция остаются прежними.

**Последнее уточнение:** цветная утка — только исходное фото сайта `public/media/web/duck-plate-960.webp`. В эскизе рабочего стола допускаются пропорциональное уменьшение, размещение и отдельная контактная тень. Еда не перерисовывается и не ретушируется; карандашным остаётся лишь её небольшой набросок на странице. [Исправленный эскиз](../design/mockups/menu-worktable-2026-09-13/real-duck/README.md). Активные токены и интерфейс прежние.

Последующее «Я передумал, давай уткц» выбирает утку вместо осьминога для той же композиции рабочего стола. Этот выбор затрагивает только содержимое растрового эскиза и согласованность блюда с наброском; общие стили и активный интерфейс прежние.

**Последний выбор:** пользователь одобрил рабочий стол и попросил осьминога. Сохраняется показанная предметная композиция и её фактура; одобренное изображение является референсом. Цветная подача и её карандашный набросок должны совпадать по составу. Текущая правка меняет только растровый эскиз; фон, шрифтовые роли и активные стили сайта остаются прежними.

Новое «Нарисуй как это будет выглядеть» относится к отдельному эскизу рабочего стола: общий светлый фон, тонкие карандашные страницы и заметки, цветные продукты и одна готовая тарелка. Подписи внутри блокнота являются частью иллюстрации; никакие шрифтовые роли действующего сайта не меняются. Прежний эскиз с людьми отклонён. Ни одна новая версия ещё не включается в активную дизайн-систему.

**Предложение для отдельного эскиза, не действующая система:** последующее «Сначала эскиз потом всё остальное» откладывает интеграцию и публикацию. Активный сайт сохраняет предыдущий цветной лист осьминога.

Прямой запрос с референсом заменяет лист осьминога открытой сценой на общем `--paper: #f4efe5`. Тонкий графит для людей и эскизов, натуральный цвет еды. Названия подач Oranienbaum не меньше 19 px, описания Ysabeau Office 16.5 px; роли PT Mono прежние. Сцена до 960 px с проходом для внешней стрелки, высота по содержимому. Desktop: четыре блюда без оболочек; до 620 px две колонки; узкий контейнер при 200% текста — одна. Нет рамки, радиуса, отдельного фона или тени; мягкие контактные тени тарелок принадлежат изображению. Связь эскиза с блюдами — тонкая нейтральная линия по референсу. Изображения статичны, содержание в HTML, ошибка и повтор доступны. Существующие фокус/hover, 40%/+55 px и breakpoint процесса 900 px сохраняются. [Аудит и план](MENU_CONCEPT_2026-09-13.md). Ниже — история.

## Цветное меню с изображением каждого ингредиента — 2026-09-13

Последние прямые запросы оставляют один постоянно цветной лист осьминога без карандаша, скролл-переходов, режимов и навигации. Сохраняются лист #f8f5ee, рамка, поля, максимальная ширина640px, действующие Oranienbaum/Ysabeau Office/PT Mono. Все десять ингредиентов имеют фотографическое изображение справа от названия, количества и действия по прежнему правилу основных компонентов; отдельный текстовый список убирается. Фото фрагментов кадрируются существующим принципом SVG clipPath, без рамок/карточек и без перерисовки. Граммовки — PT Mono с русской запятой, единица «г» — Ysabeau Office, явно указана1порция подготовленных продуктов. Текст16px, названия≥19px; изображения84px по высоте на desktop и79px на телефоне, первый осьминог крупнее как раньше. Для натурального масштаба зубчик чеснока68px, соль и перец58px; размеры статические. В узком контейнере при увеличенном тексте каждая фотография следует после своего описания. Полная тарелка ниже последнего ингредиента, доступна уже в HTML. Ссылка «Источники фото» в нижней строке листа наследует muted/подчёркивание и имеет цель44px; нижняя строка переносится на узкой ширине. [Аудит и план](MENU_BOOK_COLOR_2026-09-13.md). Ниже — предыдущая механика.

## Тонкие образы по скроллу — 2026-09-13

Последнее уточнение заменяет плотные растровые рисунки тонкими незалитыми карандашными линиями. Длина штрихов зависит от прокрутки, цвет переключается резко без fade. Предметы остаются на местах. Цветные изображения, книга, палитра, типографика и breakpoint 900 px сохраняются. Статичный просмотр поддерживает доступность и reduced motion. [План](MENU_BOOK_SCROLL_LINES_2026-09-13.md).

Линии: 0,75 CSS px независимо от масштаба, существующий muted #6e665a, opacity 0,9, круглые окончания и соединения, без заливок/серой текстуры. Штрихи следуют друг за другом с долей прокрутки, пропорциональной длине линии. Прорисовка начинается, когда центр изображения достигает 90% высоты viewport, завершается на 45,72%; цвет резко включается на 40,32%. Переход бинарный, без CSS transition. Контроллер обновляется только по scroll/resize, без таймера. При reduced motion сразу цвет; доступны оба статичных вида. Кнопки не меньше 44 px; при увеличении текста управление переносится по строкам. Ниже описана предыдущая версия с таймером.

## Последовательное проявление блюда — 2026-09-13

Последняя коррекция заменяет анимацию сборки на три фазы: карандашная прорисовка на месте, цвет ингредиентов, готовая тарелка целиком. Нет перелёта, масштабирования и движущихся копий. Цвет сохраняется в финале. Книга, типографика, палитра, рецепты и управление прежние; изображения осьминога сохраняются. [Аудит](MENU_BOOK_PENCIL_COLOR_2026-09-13.md).

Обычный начальный вид — карандаш; однократное воспроизведение начинается при видимости 30% книги. Полная последовательность 6,6 секунды: штрихи до ~2,8 с, цвет компонентов с 2,85 с, целая подача с 4,9 с. Ручное переключение сохраняет выбранный статичный вид, reduced motion исключает воспроизведение. Смена ширины внутри одного разворота не останавливает opacity-анимацию; переход через 900 px создаёт нужную страницу и отменяет прежний контроллер. Исходное фото утки показано без ретуши еды, рисунок строится по нему; каре остаётся иллюстрацией по фотореференсу. Новых общих токенов нет.

## Меню-книга в процессе — 2026-09-13

По последующему «Добавь на очновеой» проверенное решение публикуется на существующем основном сайте без изменения визуальной системы. [Выпуск](RELEASE_MENU_BOOK_2026-09-13.md).

Реализовано и проверено на семи обязательных ширинах и при 200% текста. Нативная интеграция использует правила ниже; палитра и шрифты остального сайта сохранены. [Итоговый отчёт](MENU_BOOK_SITE_2026-09-13.md).

Выбранный v2 переносится в основной сайт с действующими Oranienbaum/Ysabeau Office/PT Mono. Отдельная книга использует paper/ink/muted/gold-text/rule и свет листа#f8f5ee из макета, тонкую предметную кромку/сгиб. У каждого листа4 компонента с приготовлением и целая тарелка ниже. На desktop книга ниже рисунка01 занимает ширину процесса с проходом слева для маршрута;02 сохраняет40% сдвиг заголовка,03 идёт после книги с55px проходом,04 ниже03 на40% содержания. До900px один рецепт с перелистыванием, на узком экране книга отступает от линии процесса. Состояния: статичная цветная подача/карандаш, однократная анимация9секунд при появлении, повтор/пауза, reduced motion и ошибка/повтор. Основные описания16px+, названия≥19px, управление≥14px/44px. Габариты и интервалы относятся только к выбранному блоку. [План и проверка](MENU_BOOK_SITE_2026-09-13.md).
...
Актуальная типографика по [прямому заданию](../design/references/oranienbaum-brief-2026-09-13.md) заменяет более ранние шрифтовые записи ниже. Oranienbaum 400 normal: логотип 19px/1, uppercase/+0.22em; h1 clamp(34px,5vw,52px)/1.02; h2 clamp(26px,3.4vw,38px)/1.1; h3/названия частей блюда clamp(19px,1.5vw,20px)/1.3. Дисплейные роли не меньше 19px, только ink, без синтеза, курсива или жирности; трекинг заголовков 0. Ysabeau Office: body 400 16.5px/1.62, UI/label 500 13px/1.35/+0.02em, FAQ 600 18px/1.35 (роль предусмотрена, секции нет), все курсивные акценты 400 italic. PT Mono 400: числа 13px/1.5 с tabular/lining; 01–04 22px/+0.04em. Существующие названия частей блюда становятся дисплейными подписями; Caveat остаётся только резервной переменной, без загрузки и новых пометок.

Практическая подгонка по измерению: em Hero — Ysabeau Office italic 0.92em; при исходном 1em текст 262.70px не помещался в 246.41px с прежним внутренним отступом, а 0.92em сохраняет пять строк на всех семи ширинах. У логотипа white-space:normal устраняет переполнение 395px на экране 375px при 200% текста; при 100% он остаётся в одну строку. Короткий абзац award-title набран Ysabeau Office 500 в прежней роли подзаголовка, а не дисплейным шрифтом.

Локальные WOFF2 с двумя подмножествами Cyrillic/Latin, swap и двумя кириллическими preload Oranienbaum/Ysabeau Office 400. Палитра и нетипографические свойства остаются: контейнеры, отступы, сетки, рамки, радиусы, тени, фото, видео, маршрут 40%/+55px, состояния и breakpoints 1100/900/620. Только цвет дисплейного текста приводится к явно требуемому ink; золотая крупная строка Hero сохраняется и набрана курсивом Ysabeau Office. Текст и DOM прежние, Evgen Grebenik сохраняется. Работа только на localhost. [Аудит и план](ORANIENBAUM_2026-09-13.md).

## Фотографическая композиция меню — 2026-09-13

Прямой референс разрешает локальную сцену стены и угла стола в02. Фон#f4efe5, тени/плоскости — смеси paper/rule/muted; стрелки графитные, номер латунный. Ингредиенты — прозрачные маски настоящих фото, без генеративного достраивания. Подписи названий частей блюда, пояснения и h3 наследуют действующие display/sans-роли: параллельное более новое задание Oranienbaum/Ysabeau Office/PT Mono прямо относит названия частей блюда к Oranienbaum400. Дополнительный рукописный шрифт в итоге не подключается. Без радиусов, карточной рамки и анимации; контактная тень тарелки имеет источник в снимке. Сетка40%/+55px, остальные медиа и состояния прежние. До900px — узкая сцена с пояснениями под ней; до620px — компактные выноски без уменьшения основного текста. При ширине контейнера менее18rem (включая увеличение текста до200%) выноски переходят в полноценный одноколоночный список под изображением. Работа пока только на localhost по уточнению пользователя. [Аудит, источники и проверка](MENU_EXPLODED_PHOTOS_2026-09-13.md).

## Шеф и облако мысли — только макет, 2026-09-13

По прямому запросу создаётся увеличенная отрисовка 02: отдельный шеф из рисунка знакомства, блокнот, короткая графитная линия и облако с иллюстрацией будущего видео. Сохраняются общая бумага, тонкий карандаш, латунный номер и роли Spectral/Golos/IBM Plex Mono. Контур и взаимное расположение являются предложениями; токены, CSS, размеры, breakpoints, плееры и сайт не меняются. Новых карточек, теней, библиотек и активной анимации нет. [Источники и ограничения](../design/mockups/chef-menu-thought-2026-09-13/README.md).

## Стрелки между этапами — 2026-09-13

В процессе тонкий пунктир заменяется плавным непрерывным графитным штрихом с открытыми наконечниками. Цвет --muted, толщина 1.5px, круглые окончания/соединения; узлы и цифры сохраняют латунь. Стрелки начинаются/заканчиваются с зазором от узлов и обходят содержание. До900px — отдельные вертикальные SVG-стрелки, без прежней CSS-пунктирной линии. Нет анимации и интерактивных действий; раскладка40%/+55px, шрифты и медиа прежние. [Источник и план](PROCESS_ARROWS_2026-09-13.md).
...
## Spectral / Golos Text / IBM Plex Mono — 2026-09-13

Новая система по [прямому заданию](../design/references/typography-role-brief-2026-09-13.md) заменяет все более ранние требования единого Bellota/Caveat. Spectral400: имя/h1/h2; Spectral500: h3 и названия частей блюда; Spectral400italic: курсивные акценты. Golos Text400: абзацы,500: labels/UI,600: только роль question (FAQ отсутствует). IBM Plex Mono400: только цифры;01–04 —22px/1.50/+0.04em,25 —spec13px/1.50/+0.02em. Все числа tabular-nums lining-nums. Caveat не подключается: отдельных пометок на полях нет.

Роли desktop1440→375: display46→29px/1.10→1.14/−0.015em; h233→24px/1.18/−0.005em; h321→19px/1.30/0; copy16.5px/1.62→1.70/0; question18→17px/1.35/0; label13→12.5px/1.35/+0.02em; spec13px/1.50/+0.02em. Между крайними ширинами плавная интерполяция. Логотип сохраняет одобренные размеры24/20px (footer22/17px), меняет только гарнитуру. Пояснения dd тарелки — роль label13→12.5px; название dt — Spectral500/heading. Текст допускает аварийный перенос длинного слова; em Hero переносится только когда не помещается (проверка200%), пять строк при100% сохранены. Базовые кириллица/латиница — локальные WOFF2/swap, два preload400 для кириллицы Spectral/Golos. Синтез отключён. Палитра, контейнеры, колонки, отступы, радиусы, тени, рамки, изображения, hover/focus, animation/reduced motion и breakpoints1100/900/620 остаются прежними. [Аудит и план](TYPOGRAPHY_ROLES_2026-09-13.md).

## Три растровые концепции взаимодействия — 2026-09-13

По прямому запросу визуализированы три обсуждённые идеи. Первые две используют целиком свежий блок с графитными стрелками,40%/+55px, Spectral/Golos/IBM Plex Mono и прежней зоной02. Первая показывает один выбранный цветной объект среди рисунков; вторая — один объёмный объект и сдержанные текстовые действия. Третья предлагает одну изменяющуюся сцену стола и отдельную раскадровку, сохраняя бумагу/графит/цветную еду и смысл01–04. Все новые состояния, контрольные элементы и геометрия концептуальны: активные токены, стили, адаптив и сайт не меняются. [Источники и границы](../design/mockups/three-interactive-directions-2026-09-13/README.md).

## Идеи интерактивного меню — не внедрены, 2026-09-13

Рисунок→фотография по нажатию, объёмная сборка одного блюда и единая сцена стола предложены по прямому запросу. Отдельный разговорный пример первого поведения использует текущие шрифты/бумагу/чернила, исходные локальные изображения и временный экранный эффект рисунка. Разрешены нажатием раскрытие и обратное действие; декоративного автоматического движения нет, учитывается reduced motion. Значения перехода и увеличения — параметры примера, не активные токены. Структура01–04, текущая позиция03, библиотеки и приложение прежние. [Источники и ограничения](../design/mockups/interactive-menu-ideas-2026-09-13/README.md).

## Более спокойный визуал02 — только альтернативы, 2026-09-13

После невыбранного полного макета со слоями предлагаются A — компактные готовые подачи разного масштаба с четырьмя названиями, B — карандашный набросок меню с одной фотографической тарелкой. Оба используют прежнюю по высоте область медиа02 и весь текущий блок без сдвига03/04. Палитра, Spectral/Golos/IBM Plex Mono и общий бумажный фон сохраняются. Рисунки B основаны на графитном языке существующих сцен. Новый активный стиль, компоненты, библиотеки и адаптивные токены не вводятся. [Аудит и план](../design/mockups/calm-process-2026-09-13/README.md).

## Полный блок с четырьмя сборками — только макет, 2026-09-13

По просьбе показать весь блок готовится растровая примерка предыдущих четырёх сборок в действующий «От разговора — к вашему столу». Источник — свежий process-1440.png из typography-roles-2026-09-13. Сохраняются текущие Spectral/Golos/IBM Plex Mono, палитра,46/54 колонки,01/03 слева,02/04 справа, рисунки и видео. Увеличение высоты02 сдвигает04 по содержимому; это концептуальная геометрия, не новый CSS-токен. Программные компоненты, адаптивные правила, состояния и библиотеки прежние. [План и ограничения](../design/mockups/full-process-exploded-2026-09-13/README.md).

## Четыре блюда в разборе — только макет, 2026-09-13

Новый пользовательский [референс разнесённых слоёв](../design/references/user-exploded-food-2026-09-13.png) используется для четырёх кулинарных сборок. Текущие paper/ink/muted/gold, тонкие выноски и Spectral/Golos Text/IBM Plex Mono сохраняются. Бордовый фон и бургер не переносятся. Две свободные пары с отдельными слоями и небольшими готовыми подачами — растровое предложение; размеры, высота блока и мобильная компоновка не являются новыми активными токенами. Новые библиотеки, состояния и компоненты отсутствуют. [Аудит и границы](../design/mockups/exploded-four-courses-2026-09-13/README.md).

## Схемы меню и сборки — только макеты, 2026-09-13

Для двух увеличенных растровых фрагментов этапа02 используются действующие Bellota Text400, бумага #f4efe5, текст #0a0a0a/#6e665a, латунные линии и подписи. Вариант A переносит существующий принцип пунктирного маршрута на порядок подач. Вариант B объединяет существующий карандашный рисунок и фотографическую тарелку в предложенную схему компонентов со стрелками. Новые подзаголовки, отдельные этюды еды, геометрия и масштаб являются неутверждёнными предложениями, не активными токенами. Адаптив, состояния, библиотеки и приложение прежние. [Аудит и границы](../design/mockups/menu-schemes-2026-09-13/README.md).

## Четыре категории еды — только макет, 2026-09-13

Для отдельного растрового предложения «Продумываю меню» повторно используются действующие Bellota Text400, бумага #f4efe5, текст #0a0a0a/#6e665a, подписи #72561f и тонкие выноски. Вместо одной тарелки предложены четыре разные подачи в пределах прежней зоны медиа. Новые активные токены, программные компоненты, библиотеки, брейкпоинты или состояния не вводятся. Точные размеры и мобильная компоновка предложения не утверждены. [Источники, аудит и план](../design/mockups/menu-courses-2026-09-13/README.md).
...
## Исследование пяти шрифтов — 2026-09-13

По запросу пользователя создаются отдельные макеты Balsamiq Sans, Pangolin, Bellota Text, Kurale и Lora. Макеты используют текущие палитру, фотографии, композицию и шкалу первого экрана, заменяя только реальные файлы гарнитур в изолированном просмотре. Новые токены, стили, компоненты и библиотечные темы не утверждены и не подключаются к сайту. Текущим шрифтом остаётся Shantell Sans. [План и результаты](../design/mockups/five-fonts-2026-09-13/README.md).
...
## Исследование «Фото и подпись» — 2026-09-11

По «Нарисуй макет» создан отдельный [мобильный PNG](../design/mockups/formats-photo-captions-2026-09-11/mobile.png): номер и название в строку, подпись под фото, существующие тонкие разделители и золотистые слова «ваш вечер». Основа — текущие Caveat, бумага #f4efe5, текст #0a0a0a / #6e665a, --gold-text #72561f и --rule #c8c0b3. Это композиционное предложение; новые значения размеров, интервалов, начертаний или брейкпоинтов не приняты. Точная общая область 3:2 остаётся требованием действующей системы, но в PNG генератор воспроизводит геометрию и фото приблизительно. Состояния, библиотеки и интерфейс не меняются. [Аудит, промпты и границы проверки](../design/mockups/formats-photo-captions-2026-09-11/README.md).
...
## Домашний фон широкого снимка — 2026-09-11

Прямое уточнение пользователя заменяет профессиональный кухонный фон на окружение частного дома. Только отдельная фотографическая производная3:2, исходный шеф и предметы сохраняют пропорции; фон можно менять вокруг их силуэта. Новые UI-токены и оформление сайта не вводятся. [Аудит и промпт](../design/generated/private-dinner-home-2026-09-11/README.md). Предыдущая стальная проба — история.

## Проба расширения первого фото до3:2 — 2026-09-11

Пользователь разрешил дорисовать боковой фон private-dinner.jpg. Создаётся отдельная3:2-производная; центральная фигура и предметы должны сохранять пропорции/позу, добавленные боковины продолжают ту же кухню. Оригинал остаётся. CSS, палитра, Caveat, контейнеры, состояния и действующий сайт не меняются. [Аудит и промпт](../design/generated/private-dinner-outpaint-2026-09-11/README.md). Эта авторизация снимает запрет именно на расширение фона; замена снимка другим по-прежнему исключена.

## Исходные фото без замены — 2026-09-11

По уточнению пользователя предложение заменить фото01 отменено. Только в отдельном макете три одинаковые области3:2 обозначены тонким существующим --rule; внутри подлинные фотоcontain/center, исходные пропорции и кремовые поля. Палитра/Caveat и открытый список прежние. Новых UI-библиотек и живых компонентов нет. [Аудит и макет](../design/mockups/format-options-2026-09-11/ORIGINAL_PHOTOS.md). Следующая запись — история отклонённой замены.

## Проба одинаковых фотографий — 2026-09-11

Предложение пользователя исследуется в отдельном PNG: общая3:2-геометрия трёх фото с заменой вертикального01 на реальный горизонтальный кадр из его подборки. Структура названия/описания/фото, Caveat, бумага и линии берутся из текущей системы. Новых CSS-токенов, компонентов, состояний или библиотек нет; действующая версия сохраняется. [Аудит и промпт](../design/mockups/format-options-2026-09-11/EQUAL_PHOTOS.md). Это макет, не результат адаптивной реализации.

## Исследование пяти композиций форматов — 2026-09-11

Подготовлены только растровые мобильные предложения в [format-options-2026-09-11](../design/mockups/format-options-2026-09-11/README.md). Действующие токены и интерфейс не меняются. Использована текущая система: единый Caveat; бумага #f4efe5, текст #0a0a0a / #6e665a, номера #72561f, линии #c8c0b3; открытые группы без теней и закруглённых карточек. Пропорции фото определяют раскладку; одинаковая площадь и одинаковые контейнеры исследуются как разные понятия.

Новые значения контейнеров, сеток, отступов и размеров шрифта в PNG ориентировочные и не приняты как CSS-токены. Состояния, брейкпоинты и анимация действующей системы сохраняются. Вариант 4 предлагает функциональный текстовый переключатель: точная семантика, focus/keyboard, адаптивность и высота содержания подлежат разработке только при внедрении. Макеты не подтверждают прохождение семи ширин и не заменяют браузерную проверку. UI-библиотек нет.
...
## Меню-разворот — изолированный интерактивный макет, 2026-09-13

По прямому запросу испытывается один бумажный разворот для трёх поочерёдно выбранных блюд. Используются действующие `paper #f4efe5`, `ink #0a0a0a`, `muted #6e665a`, `gold-text #72561f`, `rule #c8c0b3` и локальные Spectral/Golos/IBM Plex Mono. Тонкая кромка и предметный сгиб основаны на метафоре реального меню; никакой сетки карточек. Новые значения геометрии относятся только к макету и не становятся активными токенами сайта. На компактной ширине — одна крупная страница с переключением, цели ≥44px, видимый focus. Анимация по действию: контур → цвет → сборка; конечная, с паузой/повтором, при reduced motion — статичное раскрытие. [Аудит, источники и проверка](ANIMATED_MENU_BOOK_2026-09-13.md).
## Одно блюдо на листе — уточнение макета, 2026-09-13

Прямое уточнение меняет смысловую компоновку рабочей пробы: на каждом листе ингредиенты с описанием операций, сборка и большая готовая тарелка. Соседний лист посвящён другому блюду. Общая бумага, книжный сгиб и типографика исходной пробы Spectral/Golos/IBM Plex Mono сохраняются; более новая система основного сайта Oranienbaum/Ysabeau Office/PT Mono не меняется. Новые изображённые блюда и настоящая растровая карандашная штриховка основаны на нашей утке и прямом запросе; рисунок проявляется функциональной маской, затем уступает цветной подаче. На телефоне один целый рецепт, без прежних отдельных страниц «Состав / Подача». Состояния паузы/повтора/reduced motion и цель44px сохраняются. В покое сверху карандаш, снизу готовая цветная тарелка; анимация длится9секунд. RGB-фото показаны через SVG-обтравку, белый фон рисунков сливается с бумагой через multiply. Исходные PNG сохранены. Это значения и медиа отдельного прототипа, проверенного на всех семи ширинах. [План и проверка](MENU_BOOK_ONE_DISH_PER_PAGE_2026-09-13.md).


## Внешность гостьи в сцене разговора — 2026-09-18

По прямому запросу женщина справа перерисовывается по двум пользовательским фотографиям. Сохраняются тонкая графитная штриховка, светлая бумага, сцена 1536 × 1024 (3:2), поза, блуза, шеф и предметы. Прежние responsive-размеры, multiply/grayscale/contrast, типографика, сетка и состояния не меняются. Новых токенов, компонентов или библиотек нет. [Аудит и проверка](CONVERSATION_GUEST_2026-09-18.md).

## Бумажное оформление видео подготовки — 2026-09-22

По прямому запросу выразительности ChefStoryVideo получает оформление из выбранного альбома: бумага #faf7f0, прежняя локальная тень rgb(72 53 29 / 11–13%), прямые углы, статический наклон в пределах выбранных фото (до −2°). Базовые шрифты, цвета, контейнеры и маршрут прежние. Верхнее и боковые поля около .625rem, нижнее шире; на телефоне поля и наклон уменьшаются, оставляя место в границах страницы. Видео остаётся 9:16/contain без цветового фильтра и обрезки. Новых hover/motion-эффектов нет. Ошибка и повтор остаются обычным читаемым интерфейсом вне оформления. [Аудит](PREPARATION_FILM_2026-09-22.md).
