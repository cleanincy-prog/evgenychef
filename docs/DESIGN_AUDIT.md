## Выпуск одобренной сцены V8 — 2026-09-11

Аудит охватывает новую иллюстрацию и её стык с бумажным фоном. По фото пользователя и двум одобренным превью заменён портрет, сохранена женщина, добавлены миницветы и два стакана. На промежуточном просмотре обнаружено пересечение линии на 1280/1024; существующий маршрут исправлен по измеренным границам сцены. Новые секции, карточки, скругления, украшения вне запроса, библиотеки и тексты-заполнители не добавлены. Дизайн-источники и состояние подтверждения записаны в DESIGN_REFERENCE_MAP и CHEF_CONVERSATION_V7_2026-09-11. Финальные результаты — CHEF_PENCIL_CURRENT_REPORT.md.
...
- **Verification:** production build and eight regression tests pass; ESLint and `git diff --check` pass.
