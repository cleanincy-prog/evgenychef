# Коллаж Hero: точное размытие 6 px — 2026-09-22

Прямое «Сделай рызмытие 6» задаёт статический blur(6px) вместо всех прежних значений. Во время движения сохраняется filter:none; остальной Hero прежний. [Аудит](docs/HERO_COLLAGE_BLUR_6PX_2026-09-22.md).

# Коллаж Hero с размытием 8 px — 2026-09-22

Последнее «Еще больше размытости» усиливает статический фильтр коллажа с 5 до 8 px. Это актуальное значение вместо прежних 2/3/5 px. Во время движения сохраняется filter:none; портрет, текст, фото и геометрия прежние. [Аудит](docs/HERO_COLLAGE_BLUR_8PX_2026-09-22.md).

# Коллаж Hero с размытием 5 px — 2026-09-22

Последнее «Еще больше размытия» усиливает статический фильтр коллажа с 3 до 5 px. Это актуальное значение вместо прежних 2/3 px. Во время движения сохраняется filter:none; портрет, текст, фото и геометрия прежние. [Аудит](docs/HERO_COLLAGE_BLUR_5PX_2026-09-22.md).

# Чуть сильнее размытие коллажа Hero — 2026-09-22

Прямое «Добавб немного больше блюра на коллаэ» увеличивает статический blur с 2 до 3 px. Во время вступления по-прежнему filter:none; портрет и текст резкие, фотографии, геометрия и прочие разделы сохраняются. Новое значение заменяет прежнее требование 2 px. [Аудит](docs/HERO_COLLAGE_BLUR_2026-09-22.md).

# Запуск Hero на iPhone — 2026-09-22

Прямое «Не отображаются на айфоне изменения» исправляет совместимость и преждевременную отмену вступления на медленной мобильной загрузке. Убрана зависимость от scripting media query; ожидаются ведущие фото и портрет, коллаж декодируется заранее с дополнительным ожиданием до 800 ms, один зависший снимок не блокирует старт. Движение по-прежнему 3 секунды. Изменение только высоты окна не отменяет его. No-JS/reduced motion/ошибка ключевого ресурса сохраняют статический Hero, позднего скрытия карточки нет. Фото, итоговая геометрия и другие разделы прежние. Публикация на существующие домен и GitHub разрешена текущими «Комить на сайт» / «на гит и домен». [Аудит и проверки](docs/HERO_IPHONE_2026-09-22.md).

# Полная сцена знакомства при сохранённом видео — 2026-09-22

Прямое «Отлично, закомить на сайт, но анимированую сцену оставь, просто дорисуй чего не хватало ниже» выбирает полный стол, стулья и ноги из одобренного рисунка этой задачи. ConversationVideo сохраняет исходный MP4, движущиеся лица/руки/реплику, mute/inline, два проигрывания и прежние fallback-состояния. Ниже и по краям добавляется совмещённый неподвижный растровый слой; вся сцена 1280×1080. Остальные разделы сохраняются. [Аудит и проверки](docs/CONVERSATION_SCENE_2026-09-22.md).

# Вступление Hero и разрешение публикации — 2026-09-22

Прямые «Комить на сайт» и «на гит и домен» разрешают выпуск проверенного локального Hero. Четыре фото из нынешнего коллажа появляются справа, летят левее в коллаж, остальные ускоряют поток; портрет и текст плавно проявляются сверху, CSS-сценарий 3 секунды. Движущиеся слои без blur; готовый коллаж сохраняет исходные 63 фото, размытие и геометрию. Reduced motion и отказ JS/ресурсов оставляют статический Hero. Остальные разделы сохраняются. [Аудит и проверки](docs/HERO_RELEASE_2026-09-22.md).

# Короткий заголовок опыта — 2026-09-22

Прямое «Убери Больше 25 лет в гастрономии, оставь больше 20 лет работы шеф поваром» оставляет заголовок «Больше 20 лет работы шеф-поваром». Часть про 25 лет удаляется целиком; фото, абзац биографии, оформление и остальные разделы сохраняются. Эта запись отменяет прежний текст только данного заголовка. [Аудит и проверки](docs/EXPERIENCE_HEADING_2026-09-22.md).

# Бумажное оформление видео подготовки — 2026-09-22

Прямое «Видео смотрится чуть чуть нелепо, просто кусок видео и все, можно его как то сделать выразительнее?» меняет только оформление ChefStoryVideo. Ролик становится живой фотографией из уже выбранного «Альбома шефа»: светлая бумажная кромка, мягкая контактная тень и статический наклон −1.8° (−1.2° на телефоне). Полный кадр 9:16, исходный ролик/постер, слова, беззвучное воспроизведение по видимости, повтор, reduced motion и остальные разделы сохраняются. Удалённая подпись не возвращается. [Аудит и проверки](docs/PREPARATION_FILM_2026-09-22.md).

# Более заметная лесенка этапов — 2026-09-22

Прямое «Блоки От разговора в вашему столу, не должны быть паралельно, они должны быть чуть ниже друг друга» увеличивает смещение правых этапов: «Продумываю меню» ниже «Знакомимся», «Ваш вечер» ниже «Готовлю к встрече». На desktop сдвиг — 60% высоты предыдущего левого этапа вместо 40%; до 900 px сохраняется вертикальный порядок. Тексты, полные кадры, тарелка, маршрут и прочие разделы сохраняются. [Аудит и проверки](docs/PROCESS_STAGGER_2026-09-22.md).

# Два просмотра видео знакомства — 2026-09-22

Прямое «видео не тормозило… начиналось когда скрол до него доходит… нельзя было его ставить на паузу прикосновением… не больше 2х раз» меняет только ConversationVideo: предварительная загрузка, старт при появлении 20% кадра, максимум два полных проигрывания за загрузку страницы, затем последний кадр. Прикосновение не управляет видео; уход за экран и reduced motion сохраняют ресурсосбережение. Без звука, кадрирования и изменения текста; другие видео имеют прежнее поведение. [Аудит и проверки](docs/CONVERSATION_PLAYBACK_2026-09-22.md).

# Убрана нумерация продуктов и этапов — 2026-09-22

Прямое «Убери все цифры 01 02 03 01 02 03 04 с сайта» удаляет три номера продуктов и четыре номера этапов вместе с их отдельными колонками. Тексты, изображения, видео, узлы и линии маршрута сохраняются; содержательные числа (25 и 20 лет, 7 подач) остаются. [Аудит и проверки](docs/NUMBERING_REMOVAL_2026-09-22.md).

# Новый абзац об авторской кухне — 2026-09-22

Прямое «Вот этот текст закинь на сайт» заменяет только award-note под заголовком об опыте. Актуальный текст начинается «В основе моей кухни — опыт лучших кулинарных школ Франции, Швейцарии и Германии…» и заканчивается «делюсь своим опытом и подходом к приготовлению». Перенос дословный; заголовок, фото, оформление и прочие разделы прежние. Эта редакция отменяет предыдущие формулировки только данного абзаца. [Точный текст, аудит и проверки](docs/BIOGRAPHY_COPY_2026-09-22.md).

# Видео тетради вместо рисунка — 2026-09-22

Прямое «Замени картинку на это видео… убери звук и сделай более плавные движения» выбирает пользовательский `Сделай_что_бы_на_этом_видео_ше.mp4` для MenuWorktable в этапе 02. Полный кадр 16:9, без аудио, с плавностью 60 fps и спокойным темпом 0.8×; без панели, inline, по видимости, с повтором. Reduced motion/no-JS оставляют постер. Внешние тексты, тарелка, остальные фото и видео сохраняются. [Аудит и проверки](docs/MENU_VIDEO_2026-09-22.md).
