...
const reports=[];
await mkdir('artifacts/oranienbaum-2026-09-13', { recursive: true });
try {
 for(const [name,width,js,delay] of [['cold-desktop',1440,true,0],['cold-mobile',375,true,0],['slow-fonts',375,true,1200],['no-js',375,false,0]]){
  ...
  if(js) await page.addInitScript(()=>{
    window.typeFrames=[];
    function tick(t){const h=document.querySelector('h1');if(h){const s=getComputedStyle(h);if(s.fontFamily.includes('Oranienbaum')){const b=h.getBoundingClientRect();window.typeFrames.push({time:t,loaded:document.fonts.check('400 29px Oranienbaum','Евгений')&&document.fonts.check('italic 400 34px "Ysabeau Office"','ваш личный'),width:b.width,height:b.height});}}if(t<6000)requestAnimationFrame(tick)}requestAnimationFrame(tick);
  });
  for(const navigation of ['cold','reload']){
   ...
   assert.ok(record.h1.includes('Oranienbaum')&&record.body.includes('Ysabeau Office'));assert.ok(record.width<=width);
   ...
   await page.screenshot({path:`artifacts/oranienbaum-2026-09-13/${name}-${navigation}.png`});reports.push(record);
  }
  await context.close();
 }
}finally{await browser.close();await writeFile('artifacts/oranienbaum-2026-09-13/loading-report.json',JSON.stringify(reports,null,2));}
