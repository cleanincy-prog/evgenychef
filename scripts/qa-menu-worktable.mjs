...
try {
  ...
  for(const width of [1440,1280,1024,768,430,390,375]) {
    ...
    const geometry=await page.evaluate(()=>{
      ...
      return {width:innerWidth,overflow:document.documentElement.scrollWidth>innerWidth,scene,conversation,stations,intersections,image:{source:image.currentSrc,loaded:image.complete&&image.naturalWidth>0,rect:rect(image),filter:getComputedStyle(image).filter,blend:getComputedStyle(image).mixBlendMode},caption:{text:document.querySelector('.menu-worktable-caption').innerText,size:parseFloat(getComputedStyle(document.querySelector('.menu-worktable-caption p')).fontSize)},fonts:{title:getComputedStyle(document.querySelector('.station-menu h3')).fontFamily,copy:getComputedStyle(document.querySelector('.menu-worktable-caption p')).fontFamily,number:getComputedStyle(document.querySelector('.station-number')).fontFamily}};
    });
    ...
    assert.equal(await page.locator('.menu-book,[data-grams]').count(),0);
  ...
  for(const width of [1440,375]){
    ...
    assert.equal(await page.locator('.menu-worktable-caption p').evaluate(e=>e.scrollWidth>e.clientWidth+1),false);
  ...
  assert.match(await errorPage.locator('.menu-worktable-caption').innerText(),/Гребешки/);
  ...
  await writeFile(path.join(out,'report.json'),JSON.stringify({passed:true,results,errors,checks:['Seven widths and route clearance','200% text on desktop and phone','Original photograph without CSS filters or cropping','No-JavaScript content and image','Image failure, keyboard retry, 44px target and focus','63 collage photos and inline video preserved']},null,2));
  console.log('Worktable checks passed');
} catch(error) {
