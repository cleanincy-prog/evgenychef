...
try {
  for (const width of [1440, 1280, 1024, 768, 430, 390, 375]) {
    ...
    try {
      ...
      await page.locator('.evening-plan').screenshot({ path: path.join(artifacts, `process-${width}.png`) });
      record.captions = await page.locator('.menu-plate').evaluate(figure => {
        ...
        return { box:box(figure), labels:[...figure.querySelectorAll('.plate-label')].map(e => ({title:e.querySelector('dt').textContent, text:e.querySelector('dd').textContent, box:box(e), font:getComputedStyle(e.querySelector('dd')).fontSize})), nextHeading:box(document.querySelector('.station-evening .station-heading')), photo:box(figure.querySelector('img')) };
      });
      const captions = record.captions;
      verify(captions.labels.length === 4, 'Four captions must be visible');
      verify(captions.labels.every(e=>e.box.left >= captions.box.left-1 && e.box.right <= captions.box.right+1 && e.box.top >= captions.box.top-1 && e.box.bottom <= captions.box.bottom+1), 'Caption outside its figure');
      verify(captions.labels.every((e,i,all)=>all.slice(i+1).every(b=>e.box.right<=b.box.left || b.box.right<=e.box.left || e.box.bottom<=b.box.top || b.box.bottom<=e.box.top)), 'Captions overlap');
      verify(captions.box.bottom <= captions.nextHeading.top, 'Plate figure overlaps next heading');
